...
type DecisionLike=BuyCalibrationInput&{strategicScore?:number;longTermScore?:number};
...
export function deriveTodayAction(d:DecisionLike,owns:boolean):TodayDecision{
  ...
  if(d.thesisLabel==="BEARISH"&&Number(d.strategicScore??0)<50&&Number(d.longTermScore??d.thesisScore)<50)return{action:owns?"SELL":"AVOID",blocked:true,reason:"Long-term, strategic and fundamental evidence are aligned negatively enough to block new capital.",policyVersion:TODAY_POLICY_VERSION};
  const timing=d.timing?.label||"WAIT";
  const buyAudit=evaluateBuyCalibration(d);
  if(owns){
    if(d.thesisState==="Weakening"&&d.thesisScore<50&&Number(d.strategicScore??0)<50&&Number(d.longTermScore??d.thesisScore)<50)return{action:"TRIM",blocked:false,reason:"Long-term, strategic and current thesis evidence are all weakening enough to reduce exposure while evidence is reassessed.",policyVersion:TODAY_POLICY_VERSION,buyAudit};
