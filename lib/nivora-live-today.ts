...
export function applyLiveQuoteToToday(today:TodayDecision|undefined,quote:NivoraLiveQuote|null,owns:boolean):TodayDecision|undefined{
 ...
 if(quote.integrityTradable===false&&(today.action==="BUY"||today.action==="ADD")){
  return{...today,action:owns?"HOLD":"WAIT",blocked:false,reason:`Market-data integrity is ${quote.integrityState||"not verified"}. AURYN will not add risk until a fresh trustworthy quote is available.`,policyVersion:TODAY_POLICY_VERSION};
 }

 if(quote.freshness!=="LIVE"&&(today.action==="BUY"||today.action==="ADD")){
  return{...today,action:owns?"HOLD":"WAIT",blocked:false,reason:"The current quote is stale or last-trade context. AURYN requires a fresh quote before adding risk.",policyVersion:TODAY_POLICY_VERSION};
 }
 ...
 if(extended&&gap>=8&&(today.action==="BUY"||today.action==="ADD")){
  return{...today,action:owns?"HOLD":"WAIT",blocked:false,reason:`Extended-hours move is ${quote.changePct!>=0?"+":""}${quote.changePct?.toFixed(1)}%. AURYN will not chase a large gap before regular-session liquidity confirms the price.`,policyVersion:TODAY_POLICY_VERSION};
 }
