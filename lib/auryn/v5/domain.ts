...
export type MetricFamily='TREND'|'MOMENTUM'|'VOLUME_FLOW'|'VOLATILITY'|'STRUCTURE'|'RELATIVE_STRENGTH'|'BUSINESS'|'FUNDAMENTALS'|'VALUATION'|'THESIS'|'MOAT'|'CATALYSTS'|'SECTOR'|'RISK';
...
export interface ScenarioMap {structure:string;setup:string;confluenceScore:number;waveContext:{label:string;confidence:'HIGH'|'MEDIUM'|'LOW';note:string};bull:ScenarioLeg;base:ScenarioLeg;bear:ScenarioLeg;}
export interface PriceZone {label:string;low:number;high:number;multiplier:number;basis:string;}
export interface ExecutionPlan {snapshotId:string;state:'READY'|'BLOCKED';reason:string;currentPrice:number|null;initialEntry:PriceZone|null;dcaZones:PriceZone[];confirmation:number|null;invalidation:number|null;targets:Array<{label:string;price:number}>;riskPerShare:number|null;}
