...
import type {ExecutionPlan,PatternSignal,ScenarioMap} from './domain';
const clamp=(n:number)=>Math.max(0,Math.min(100,n));
const confidence=(n:number):'HIGH'|'MEDIUM'|'LOW'=>n>=75?'HIGH':n>=55?'MEDIUM':'LOW';
export function buildScenarioMap({technical,patterns,executionPlan}:{technical:TechnicalSnapshot|null;patterns:PatternSignal[];executionPlan?:ExecutionPlan|null}):ScenarioMap|null{
  ...
  const bull={
    label:'BULL' as const,summary:bullSummary,
