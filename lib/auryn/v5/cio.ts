...
import type {AurynV4CoreAnalysis,Horizon,PrimaryInvestmentAction} from '../v4/domain';
import type {TechnicalSnapshot} from '../../nivora-technical-engine';
import type {AurynV5Decision} from './domain';

const horizons:Horizon[]=['NOW','SWING','SIX_TO_TWELVE_MONTHS','THREE_TO_FIVE_YEARS'];
const act=(score:number):PrimaryInvestmentAction=>score>=84?'STRONG_BUY':score>=68?'BUY':score>=50?'HOLD':score>=36?'REDUCE':'SELL';
const finite=(x:unknown):x is number=>typeof x==='number'&&Number.isFinite(x);
export function resolveV5CioDecision({v4,technical,marketTruth}:{v4:AurynV4CoreAnalysis;technical:TechnicalSnapshot|null;marketTruth:CanonicalMarketSnapshot}):AurynV5Decision{
  const thesis=v4.thesis.strength,dir=v4.thesis.direction;const vf=v4.factors.VALUATION;const valDecisionGrade=Boolean(vf&&vf.score!=null&&!(vf.validationState!=='MEASURED'&&/preliminary|not allowed|unsupported|unavailable/i.test(String(vf.reason||''))));const val=valDecisionGrade?vf!.score:null;const risk=v4.factors.RISK?.score??null;const business=v4.factors.BUSINESS_QUALITY?.score??50;const growth=v4.factors.GROWTH_INFLECTION?.score??50;const fund=v4.factors.FUNDAMENTALS_EARNINGS?.score??50;const tech=technical?.technicalState.strength??v4.factors.TECHNICALS?.score??50;
  const hardSell=v4.primaryAction==='SELL'||dir==='BROKEN'||(finite(thesis)&&thesis<30);
  if(hardSell){
    const h=horizons.map(horizon=>({horizon,action:'SELL' as const,confidence:v4.confidence,reasonCodes:['STRUCTURAL_THESIS_BROKEN']}));
  ...
  if(thesis==null){
    const h=horizons.map(horizon=>({horizon,action:'INSUFFICIENT_EVIDENCE' as const,confidence:v4.confidence,reasonCodes:['STRUCTURAL_EVIDENCE_MISSING']}));
  ...
  const capStrongBuy=(action:PrimaryInvestmentAction):PrimaryInvestmentAction=>action==='STRONG_BUY'&&!highDecisionConfidence?'BUY':action;
  let longAction=capStrongBuy(act(longScore));
  if(dir==='WEAKENING'&&longAction==='STRONG_BUY')longAction='BUY';
  if(dir==='WEAKENING'&&longAction==='BUY'&&thesis<62)longAction='HOLD';
  if(val==null&&(longAction==='BUY'||longAction==='STRONG_BUY'))longAction='HOLD';
  if(finite(risk)&&risk>=88&&(longAction==='BUY'||longAction==='STRONG_BUY'))longAction='HOLD';
  if(longAction==='SELL')longAction='REDUCE';
  ...
  if((longAction==='BUY'||longAction==='STRONG_BUY')&&['SELL','REDUCE'].includes(nowAction))nowAction='HOLD';
  if(longAction==='HOLD'&&nowAction==='SELL')nowAction='REDUCE';
  ...
  if((longAction==='BUY'||longAction==='STRONG_BUY')&&swingAction==='SELL')swingAction='HOLD';
  if(longAction==='HOLD'&&swingAction==='SELL')swingAction='REDUCE';
  ...
  let primary:PrimaryInvestmentAction=val==null?'HOLD':longAction;
  if(primary==='STRONG_BUY'||primary==='BUY'){
    ...
    if(primary==='STRONG_BUY'&&nowPositive&&swingPositive)primary='STRONG_BUY';
