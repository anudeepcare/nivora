...
export async function loadV934LiveContext(input:{symbol:string;key:string;asOf:Date;fetchJson:Fetcher}){
  ...
  const preview:TimeframeBarSet={};
  if(all15.length){preview['15M']=all15;preview['1H']=resampleSequential(all15,4,true);}
  if(all4h.length)preview['4H']=all4h;
  return{confirmed:{'15M':d15,'1H':h1,'4H':h4} as TimeframeBarSet,preview,coverage:{'15M':d15.length,'1H':h1.length,'4H':h4.length},raw15,raw4h,errors:{'15M':fifteen.status==='rejected'?String(fifteen.reason):null,'4H':four.status==='rejected'?String(four.reason):null}};
}
