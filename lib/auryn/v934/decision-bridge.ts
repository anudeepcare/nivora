import type {AurynMarketIntelligenceSnapshot,AurynTimeframe,TimeframeTechnicalState} from './domain';

const clamp=(n:number)=>Math.max(0,Math.min(100,Math.round(Number.isFinite(n)?n:50)));
const ratingLabel=(tf:AurynTimeframe,s:TimeframeTechnicalState|null|undefined)=>s?`${tf} ${s.rating} (${s.counts.buy} buy / ${s.counts.neutral} neutral / ${s.counts.sell} sell)`:`${tf} unavailable`;
function weighted(snapshot:any,selector:(s:TimeframeTechnicalState)=>number){
 const specs:[AurynTimeframe,number][]=[['1D',.4],['1W',.25],['4H',.25],['1H',.1]];
 let total=0,denom=0;
 for(const [tf,w] of specs){const s=snapshot?.confirmed?.[tf] as TimeframeTechnicalState|undefined;if(!s)continue;const n=Number(selector(s));if(!Number.isFinite(n))continue;total+=n*w;denom+=w;}
 return denom?clamp(total/denom):50;
}
export function deriveV934DecisionTechnical(snapshot:AurynMarketIntelligenceSnapshot|any){
 const daily=snapshot?.confirmed?.['1D'] as TimeframeTechnicalState|undefined;
 const four=snapshot?.confirmed?.['4H'] as TimeframeTechnicalState|undefined;
 const weekly=snapshot?.confirmed?.['1W'] as TimeframeTechnicalState|undefined;
 const live=snapshot?.livePreview?.['1D'] as TimeframeTechnicalState|undefined;
