...
export type FastResearchQuote={
 symbol:string;price:number;changePct:number|null;provider:"alpaca"|"twelvedata-price";providerTimestamp:string|null;retrievedAt:string;latencyMs:number;
...
export const isCryptoSymbol=(s:string)=>/^(BTC|ETH|SOL|DOGE|XRP|ADA|AVAX|LINK|LTC|BCH|DOT|MATIC|SHIB)[\/-](USD|USDT)$/i.test(s);
...
export async function loadFastResearchQuote(input:{symbol:string;twelveKey?:string;alpacaKey?:string;alpacaSecret?:string;asOf?:Date}):Promise<FastResearchQuote>{
 const symbol=String(input.symbol||"").toUpperCase(),asOf=input.asOf??new Date(),started=Date.now(),crypto=isCryptoSymbol(symbol);if(!symbol)throw new Error("Symbol is required.");
 ...
 const chosen=pool.find(q=>q.provider===authority.source&&q.providerTimestamp===authority.asOf)??pool[0],baseProvider=chosen.provider.startsWith("alpaca")?"alpaca":"twelvedata-price";
