import {bootstrapMeanCI} from '../../nivora-statistics';
import {normalTwoSidedP,pearson} from '../v9/statistics';
import type {FeatureObservation,ResearchHorizon} from '../v9/domain';
import {createPurgedWalkForwardFolds} from './folds';
import type {V93FeatureMetric,V93Policy} from './domain';

const finite=(x:unknown):x is number=>typeof x==='number'&&Number.isFinite(x);
const mean=(xs:number[])=>xs.length?xs.reduce((a,b)=>a+b,0)/xs.length:0;
const round=(x:number,d=6)=>+x.toFixed(d);
function stableHash32(text:string){let h=2166136261>>>0;for(let i=0;i<text.length;i++){h^=text.charCodeAt(i);h=Math.imul(h,16777619)>>>0;}return h>>>0;}
function edge(row:FeatureObservation,costMultiplier=1){
...
function groupStability(rows:FeatureObservation[],key:(r:FeatureObservation)=>string|null|undefined,minN:number){
  const groups=new Map<string,FeatureObservation[]>();
  for(const row of rows){const k=key(row);if(!k)continue;const a=groups.get(k)??[];a.push(row);groups.set(k,a);}
  const eligible=[...groups.entries()].filter(([,a])=>a.length>=minN).sort((a,b)=>a[0].localeCompare(b[0]));
  if(!eligible.length)return{covered:0,positivePct:null as number|null};
...
function tPValue(xs:number[]){
  ...
  return normalTwoSidedP(t);
}

export function evaluateV93Feature(featureId:string,observations:FeatureObservation[],policy:V93Policy):V93FeatureMetric{
  ...
  const horizon=(rows[0]?.horizon??null) as ResearchHorizon|null;
  const folds=horizon?createPurgedWalkForwardFolds(rows,horizon,policy):[];
  ...
  const oos:FeatureObservation[]=[];
  for(const f of validFolds)for(const r of f.testRows){const k=`${r.asOf}|${r.symbol}`;if(!seen.has(k)){seen.add(k);oos.push(r);}}
  ...
  const ci=bootstrapMeanCI(oosEdges,policy.bootstrapIterations,policy.bootstrapConfidence,(policy.seed^stableHash32(featureId))>>>0);
  ...
  return{
    ...
    oosConfidence95:ci,
    oosInformationCoefficient:oos.length>=2?pearson(signals,rawAlpha):null,
