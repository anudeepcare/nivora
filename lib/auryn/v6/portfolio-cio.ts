...
export type PortfolioAction='ADD'|'HOLD'|'REDUCE_EXPOSURE'|'BLOCK_ADD';
export interface PortfolioCioOverlay{
...
export function applyPortfolioCioOverlay({independentAction,portfolioRisk,owns,currentPositionPct=0,sameArchetypeExposurePct=0}:{independentAction:PrimaryInvestmentAction;portfolioRisk:PortfolioRisk;owns:boolean;currentPositionPct?:number;sameArchetypeExposurePct?:number;}):PortfolioCioOverlay{
  ...
  if(sameArchetypeExposurePct>=35){max=Math.min(max,1.5);constrained=true;reasons.push('Archetype exposure is elevated; cap any add at a small satellite size.');}
  if(owns&&currentPositionPct>=10){max=Math.min(max,1);constrained=true;reasons.push('Existing position is already meaningful; only a small add is allowed.');}
  if(portfolioRisk.sizingGate==='REDUCED')reasons.push('Portfolio risk requires reduced sizing.');
  if(!reasons.length)reasons.push('Portfolio context does not currently constrain the standalone bullish call.');
