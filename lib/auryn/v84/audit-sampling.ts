export type AuditUniverseRow={symbol:string;market_cap_m?:number|null};
...
function clean(rows:AuditUniverseRow[]){
  const seen=new Set<string>(),out:AuditUniverseRow[]=[];
...
function hashSort<T extends AuditUniverseRow>(rows:T[]){return [...rows].sort((a,b)=>hash(a.symbol)-hash(b.symbol)||a.symbol.localeCompare(b.symbol));}
function splitRanked(rows:AuditUniverseRow[],bucketCount:number){
  const out:AuditUniverseRow[][]=[];
  for(let i=0;i<bucketCount;i++){
    const start=Math.floor(i*rows.length/bucketCount),end=Math.floor((i+1)*rows.length/bucketCount);
    if(end>start)out.push(hashSort(rows.slice(start,end)));
  }
  return out;
}

export function selectDiversifiedAuditUniverse(rows:AuditUniverseRow[],limit:number):string[]{
  const unique=clean(rows),wanted=Math.max(0,Math.min(Math.floor(limit),unique.length));
  if(wanted===0)return[];
  if(unique.length<=wanted)return hashSort(unique).map(x=>x.symbol);
  ...
  const positions=new Array(buckets.length).fill(0),selected:string[]=[],used=new Set<string>();
  ...
  while(selected.length<wanted){
    ...
    for(let b=0;b<buckets.length&&selected.length<wanted;b++){
      while(positions[b]<buckets[b].length){
        ...
        used.add(row.symbol);selected.push(row.symbol);advanced=true;break;
      }
    }
    if(!advanced)break;
  }
  if(selected.length<wanted){
    for(const row of hashSort(unique)){if(selected.length>=wanted)break;if(!used.has(row.symbol)){used.add(row.symbol);selected.push(row.symbol);}}
  }
  return selected;
}
