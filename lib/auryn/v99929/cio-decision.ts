...
export function buildCioDecision(x:any){
 ...
 const longTermAction=!structuralTruthValid?"STRUCTURE REBUILDING":(business??0)>=80&&(lt??0)>=60?"ACCUMULATE":(business??0)>=65&&(lt??0)>=45?"HOLD / SELECTIVE":(business??0)<45?"AVOID":"WATCH";
 const inEntry=price!=null&&n(x.entryLow)!=null&&n(x.entryHigh)!=null&&price>=Number(x.entryLow)&&price<=Number(x.entryHigh);
 const newMoneyAction=!structuralTruthValid?"WAIT":longTermAction==="AVOID"?"DO NOT BUY":inEntry&&(timing??0)>=55?"START SMALL":(timing??0)>=70?"BUY NOW":"WAIT";
 const ownerAction=!structuralTruthValid?"HOLD / VERIFY STRUCTURE":longTermAction==="AVOID"?"REDUCE / EXIT":(business??0)>=75?"HOLD / ADD ON WEAKNESS":"HOLD";
 // Actionable zones are intentionally narrow; distant structural levels never get merged into one giant range.
 const starterZone=structuralTruthValid?zone(price??x.entryHigh,5):null;
 const primaryCenter=n(x.longTerm?.fib618)??n(x.entryLow)??price;
 const primaryBuyZone=structuralTruthValid?zone(primaryCenter,8):null;
