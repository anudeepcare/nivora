...
import type {V92AssemblyInput} from './domain';

const sortFacts=(a:PointInTimeMetric,b:PointInTimeMetric)=>a.symbol.localeCompare(b.symbol)||a.availableAt.localeCompare(b.availableAt)||a.metric.localeCompare(b.metric)||(a.periodEnd??'').localeCompare(b.periodEnd??'')||a.value-b.value;
export function assembleHistoricalReplayBundle(input:V92AssemblyInput):HistoricalReplayBundle{
  if(!input?.datasetId||!input?.source||!input?.benchmarkSymbol)throw new Error('V9.2 assembly requires datasetId, source, and benchmarkSymbol.');
