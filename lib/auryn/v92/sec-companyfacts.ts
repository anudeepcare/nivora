...
type ConceptSpec={metric:string;units:string[];priority:number};
...
type SecMetric=PointInTimeMetric&{
  form?:string|null;
  fp?:string|null;
...
function entriesForConcept(concept:any,spec:ConceptSpec){
  const out:any[]=[];
  for(const unit of spec.units){const rows=concept?.units?.[unit];if(Array.isArray(rows))out.push(...rows);}
  return out;
}

function isAnnualForm(r:SecMetric){
...
export function normalizeSecCompanyFacts(symbol:string,payload:any):SecMetric[]{
  const s=String(symbol||'').trim().toUpperCase();if(!s)throw new Error('SEC symbol is required.');
  if(!payload?.facts||typeof payload.facts!=='object')throw new Error(`SEC Company Facts response for ${s} has no facts object.`);
  const rows:SecMetric[]=[];
  for(const taxonomy of ['us-gaap','ifrs-full']){
    const facts=payload.facts[taxonomy];if(!facts)continue;
    for(const [conceptName,spec] of Object.entries(CONCEPTS)){
      const concept=facts[conceptName];if(!concept)continue;
      for(const row of entriesForConcept(concept,spec)){
        if(row?.val==null||!finite(Number(row.val)))continue;
        if(!dateLike(row?.filed))throw new Error(`SEC fact ${conceptName} for ${s} is missing filed date; public availability cannot be inferred from period end.`);
  ...
  const best=new Map<string,SecMetric>();
  for(const r of rows){
    const k=[r.symbol,r.metric,r.periodEnd??'',r.availableAt,r.form??'',r.fp??'',r.accession??''].join('|');
    const prev=best.get(k);
    if(!prev||(r.priority??99)<(prev.priority??99))best.set(k,r);
  }
...
const pct=(a:number,b:number)=>b!==0?(a/b-1)*100:NaN;
const ratio=(a:number,b:number,scale=1)=>b!==0?a/b*scale:NaN;

function latestForPeriod(rows:SecMetric[],metric:string,periodEnd:string,availableAt:string){
...
function annualPeriodEnds(rows:SecMetric[],metric:string,availableAt:string){
  return [...new Set(rows.filter(r=>r.metric===metric&&r.periodEnd&&r.availableAt<=availableAt&&isAnnualForm(r)).map(r=>r.periodEnd as string))].sort();
}

export function deriveSecAnnualMetrics(symbol:string,raw:SecMetric[]):PointInTimeMetric[]{
  ...
  for(const anchor of anchors){
    const availableAt=anchor.availableAt,period=anchor.periodEnd as string;
    const periods=annualPeriodEnds(raw,'revenue',availableAt);const idx=periods.indexOf(period);if(idx<0)continue;
    const priorPeriod=idx>0?periods[idx-1]:null,prior2Period=idx>1?periods[idx-2]:null;
    const currentRevenue=latestForPeriod(raw,'revenue',period,availableAt);if(!currentRevenue)continue;
    const emit=(metric:string,value:number)=>{if(finite(value))out.push({symbol:s,metric,value:+value.toFixed(6),periodEnd:period,availableAt});};
    if(priorPeriod){const prior=latestForPeriod(raw,'revenue',priorPeriod,availableAt);if(prior){const g=pct(currentRevenue.value,prior.value);emit('revenue_growth',g);if(prior2Period){const prior2=latestForPeriod(raw,'revenue',prior2Period,availableAt);if(prior2)emit('revenue_acceleration',g-pct(prior.value,prior2.value));}}}
    const get=(metric:string)=>latestForPeriod(raw,metric,period,availableAt);
    ...
    if(gross)emit('gross_margin',ratio(gross.value,currentRevenue.value,100));
    if(op)emit('operating_margin',ratio(op.value,currentRevenue.value,100));
    if(cfo){const fcf=cfo.value-(capex?.value??0);emit('fcf_margin',ratio(fcf,currentRevenue.value,100));if(ni)emit('cash_conversion',ratio(cfo.value,Math.abs(ni.value)));}
    if(ni&&eq)emit('roe',ratio(ni.value,eq.value,100));
    if(liab&&assets)emit('leverage',ratio(liab.value,assets.value));
    if(ca&&cl)emit('liquidity',ratio(ca.value,cl.value));
    if(cash){const td=(debtN?.value??0)+(debtC?.value??0)+(debt?.value??0);emit('net_cash_debt',cash.value-td);}
    if(priorPeriod){
      const p=(metric:string)=>latestForPeriod(raw,metric,priorPeriod,availableAt);
      const pe=p('eps_diluted'),ps=p('shares_outstanding'),pcfo=p('operating_cash_flow'),pcap=p('capex');
      if(eps&&pe)emit('eps_growth',pct(eps.value,pe.value));
      if(shares&&ps)emit('dilution_rate',pct(shares.value,ps.value));
      if(cfo&&pcfo){const fcf=cfo.value-(capex?.value??0),pfcf=pcfo.value-(pcap?.value??0);emit('fcf_growth',pct(fcf,pfcf));}
    }
  }
  const byKey=new Map<string,PointInTimeMetric>();
  for(const r of out.sort((a,b)=>a.availableAt.localeCompare(b.availableAt)||a.metric.localeCompare(b.metric)||(a.periodEnd??'').localeCompare(b.periodEnd??'')))byKey.set([r.symbol,r.metric,r.periodEnd??'',r.availableAt].join('|'),r);
  return[...byKey.values()];
}

export function normalizeAndDeriveSecCompanyFacts(symbol:string,payload:any){
