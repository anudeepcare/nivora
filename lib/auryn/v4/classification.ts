...
function classifyBusinessModel(input:SecurityClassificationInput){
  ...
  if(industry.includes("health care technology")||industry.includes("healthcare technology")||industry.includes("health information"))return{model:"DIGITAL_HEALTH_PLATFORM" as BusinessModel,specific:true};
...
function lifecycleOf(x:SecurityClassificationInput,businessModel:BusinessModel):LifecycleStage{
  ...
  if(x.profitable===false&&["SPACE_SATELLITE","BIOTECH_PHARMA"].includes(businessModel)){
    if(growth!=null&&growth>40)return "VALIDATION";
    if(/commercial deployment|commercialization|launch milestone|carrier partner|clinical trial|clinical[- ]stage|regulatory milestone/.test(t))return "VALIDATION";
    if(rev!=null&&rev>0)return "VALIDATION";
    return "UNKNOWN";
  }
  ...
  if(growth!=null&&growth<7&&x.profitable===true)return "MATURITY";
  return "UNKNOWN";
}
...
function capitalIntensityOf(model:BusinessModel):SecurityClassification["capitalIntensity"]{
  ...
  if(["SAAS_SOFTWARE","MARKETPLACE_ADTECH","FINTECH_PAYMENTS","DIGITAL_HEALTH_PLATFORM"].includes(model))return "LOW";
  return "UNKNOWN";
}
...
function cyclicalityOf(model:BusinessModel):SecurityClassification["cyclicality"]{
  ...
  if(["SAAS_SOFTWARE","MARKETPLACE_ADTECH","FINTECH_PAYMENTS","DIGITAL_HEALTH_PLATFORM","BANK","REIT","CONSUMER","AI_DATA_CENTER_INFRA","SPACE_SATELLITE"].includes(model))return "MODERATE";
  return "UNKNOWN";
}
