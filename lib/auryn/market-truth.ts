...
export function buildCanonicalMarketSnapshot(input:{symbol:string;asOf?:Date;primary?:ExecutionQuote|null;secondary?:ExecutionQuote|null;regularClose?:number|null;regularCloseTimestamp?:string|null;maxDisagreementPct?:number}):CanonicalMarketSnapshot{
  ...
  if(calendar.session==="REGULAR"||calendar.session==="PRE_MARKET"||calendar.session==="AFTER_HOURS"){
    if(integrity.state==="LIVE_VERIFIED"&&integrity.chosen){
      priceState="LIVE_VERIFIED";decisionPrice=integrity.chosen.price;displayPrice=decisionPrice;decisionPriceAsOf=integrity.chosen.providerTimestamp??asOf.toISOString();decisionPriceRole="LIVE_MARKET";
      liveMarketPrice=decisionPrice;liveMarketPriceAsOf=decisionPriceAsOf;priceSensitiveAllowed=true;executionTradable=true;executionPrice=decisionPrice;executionPriceAsOf=decisionPriceAsOf;priceUse="LIVE_EXECUTION";
      ...
      if(closeGap!=null&&closeGap>30){
        ...
        priceState="LIVE_SINGLE_SOURCE";decisionPrice=integrity.chosen.price;displayPrice=decisionPrice;decisionPriceAsOf=integrity.chosen.providerTimestamp??asOf.toISOString();decisionPriceRole="LIVE_MARKET";
        liveMarketPrice=decisionPrice;liveMarketPriceAsOf=decisionPriceAsOf;priceSensitiveAllowed=true;executionTradable=false;priceUse="RESEARCH_LIVE_SINGLE_SOURCE";
    ...
    priceState="OFFICIAL_CLOSE";decisionPrice=regularClose;displayPrice=regularClose;decisionPriceAsOf=regularCloseAsOf;decisionPriceRole="REGULAR_CLOSE";priceSensitiveAllowed=true;executionTradable=false;priceUse="RESEARCH_CLOSE";
