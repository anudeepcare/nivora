...
export type CanonicalMarketTruth={
 ...
 confidence:string; ageSeconds:number|null; label:string; referencePrice:number|null; referencePriceTimestamp:string|null;
};
...
export function buildMarketTruth(q:any,canonical:any,checkedAt=new Date().toISOString()):CanonicalMarketTruth{
 ...
 if((session==="AFTER_HOURS"||session==="PRE_MARKET")&&price==null){freshness="VERIFYING";label=session==="AFTER_HOURS"?"AFTER-HOURS PRICE UNAVAILABLE":"PRE-MARKET PRICE UNAVAILABLE"}
 else if(session==="CRYPTO_24X7"){freshness="CRYPTO_24X7";label="24/7 LIVE"}
 ...
 const referencePrice=Number.isFinite(Number(canonical?.regularClosePrice??canonical?.regularClose))?Number(canonical?.regularClosePrice??canonical?.regularClose):null,referencePriceTimestamp=canonical?.regularCloseAsOf||null;
 return{price,provider:q?.provider||canonical?.source||null,priceTimestamp,checkedAt,session,freshness,confidence:String(q?.confidence||canonical?.confidence||"SINGLE_SOURCE"),ageSeconds:a,label,referencePrice,referencePriceTimestamp};
}
...
export function formatMarketTruth(t:CanonicalMarketTruth){const reference=t.referencePrice!=null?` · regular close $${t.referencePrice.toFixed(2)}${t.referencePriceTimestamp?` at ${formatLocalTime(t.referencePriceTimestamp)}`:""}`:"";return{status:t.label,detail:t.priceTimestamp?`Price as of ${formatLocalTime(t.priceTimestamp)} · ${t.provider||"market provider"} · ${formatAge(t.ageSeconds)} · checked ${formatLocalTime(t.checkedAt)}`:`Checked ${formatLocalTime(t.checkedAt)}${reference} · waiting for a current session price`}}
