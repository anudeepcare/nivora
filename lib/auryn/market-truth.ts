...
export function buildCanonicalMarketSnapshot(input:{symbol:string;asOf?:Date;primary?:ExecutionQuote|null;secondary?:ExecutionQuote|null;regularClose?:number|null;regularCloseTimestamp?:string|null;maxDisagreementPct?:number}):CanonicalMarketSnapshot{
  ...
  let priceState:CanonicalPriceState="UNAVAILABLE",decisionPrice:number|null=null,displayPrice:number|null=null,extendedPrice:number|null=null,decisionPriceAsOf:string|null=null,priceSensitiveAllowed=false;
  ...
  if(calendar.session==="REGULAR"||calendar.session==="PRE_MARKET"||calendar.session==="AFTER_HOURS"){
    if(integrity.state==="LIVE_VERIFIED"&&integrity.chosen){
      priceState="LIVE_VERIFIED";decisionPrice=integrity.chosen.price;displayPrice=decisionPrice;decisionPriceAsOf=integrity.chosen.providerTimestamp??asOf.toISOString();priceSensitiveAllowed=true;reason="Independent fresh providers agree within the market-truth tolerance.";
      ...
      if(closeGap!=null&&closeGap>30){
        ...
        priceState="LIVE_SINGLE_SOURCE";decisionPrice=integrity.chosen.price;displayPrice=decisionPrice;decisionPriceAsOf=integrity.chosen.providerTimestamp??asOf.toISOString();priceSensitiveAllowed=true;reason="One fresh provider is available without a contradictory fresh provider; confidence is reduced.";
      }
    ...
    priceState="OFFICIAL_CLOSE";decisionPrice=regularClose;displayPrice=regularClose;decisionPriceAsOf=input.regularCloseTimestamp??null;priceSensitiveAllowed=true;
  ...
  return{snapshotId,symbol,asOf:asOf.toISOString(),session:calendar.session,calendarState:calendar.calendarState,priceState,decisionPrice,displayPrice,regularClose,extendedPrice,decisionPriceAsOf,providerAgreementPct,sources,priceSensitiveAllowed,decisionAllowed:priceSensitiveAllowed,reason};
}
