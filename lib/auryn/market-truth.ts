...
export type CanonicalMarketSnapshot={
  ...
  priceState:CanonicalPriceState;decisionPrice:number|null;displayPrice:number|null;regularClose:number|null;extendedPrice:number|null;
  providerAgreementPct:number|null;sources:CanonicalMarketSource[];priceSensitiveAllowed:boolean;decisionAllowed:boolean;reason:string;
};
...
export function buildCanonicalMarketSnapshot(input:{symbol:string;asOf?:Date;primary?:ExecutionQuote|null;secondary?:ExecutionQuote|null;regularClose?:number|null;maxDisagreementPct?:number}):CanonicalMarketSnapshot{
  ...
  const providerAgreementPct=integrity.disagreementPct==null?null:round4(integrity.disagreementPct);
  let priceState:CanonicalPriceState="UNAVAILABLE",decisionPrice:number|null=null,displayPrice:number|null=null,extendedPrice:number|null=null,priceSensitiveAllowed=false;
  ...
  if(calendar.session==="REGULAR"||calendar.session==="PRE_MARKET"||calendar.session==="AFTER_HOURS"){
    if(integrity.state==="LIVE_VERIFIED"&&integrity.chosen){
      priceState="LIVE_VERIFIED";decisionPrice=integrity.chosen.price;displayPrice=decisionPrice;priceSensitiveAllowed=true;reason="Independent fresh providers agree within the market-truth tolerance.";
      ...
      if(closeGap!=null&&closeGap>30){
        ...
        priceState="LIVE_SINGLE_SOURCE";decisionPrice=integrity.chosen.price;displayPrice=decisionPrice;priceSensitiveAllowed=true;reason="One fresh provider is available without a contradictory fresh provider; confidence is reduced.";
      }
    ...
    priceState="OFFICIAL_CLOSE";decisionPrice=regularClose;displayPrice=regularClose;priceSensitiveAllowed=true;
  ...
  const snapshotId=`${symbol}-${calendar.date}-${fingerprint(JSON.stringify({priceState,decisionPrice,regularClose,providerAgreementPct,sources:sources.map(s=>[s.provider,s.price,s.providerTimestamp])}))}`;
  return{snapshotId,symbol,asOf:asOf.toISOString(),session:calendar.session,calendarState:calendar.calendarState,priceState,decisionPrice,displayPrice,regularClose,extendedPrice,providerAgreementPct,sources,priceSensitiveAllowed,decisionAllowed:priceSensitiveAllowed,reason};
}
