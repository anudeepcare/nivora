...
import {normalTwoSidedP,pearson} from './statistics';
import type {FeatureMetric,FeatureObservation} from './domain';
const finite=(x:unknown):x is number=>typeof x==='number'&&Number.isFinite(x);
const mean=(xs:number[])=>xs.length?xs.reduce((a,b)=>a+b,0)/xs.length:0;
const round=(x:number,d=3)=>+x.toFixed(d);
export function evaluateFeature(featureId:string,observations:FeatureObservation[],options:{inSampleFraction?:number}={}):FeatureMetric{
 ...
 const ci=bootstrapMeanCI(oosEdges,Math.min(2000,Math.max(500,oosEdges.length*20)),.95,909);
 ...
 const stable=regimes.map(reg=>{const xs=enriched.filter(x=>x.regime===reg).map(x=>x.edge);return xs.length>=5?mean(xs)>0:null}).filter((x):x is boolean=>x!==null);
 const dd=enriched.map(x=>x.maxDrawdownPct).filter(finite);
 return{featureId,totalN:enriched.length,inSampleN:ins.length,outOfSampleN:oos.length,avgEdgePct:enriched.length?round(mean(edges)):null,outOfSampleAvgEdgePct:oos.length?round(mean(oosEdges)):null,outOfSampleConfidence95:ci,informationCoefficient:pearson(signals,rawAlpha),hitRatePct:enriched.length?round(edges.filter(x=>x>0).length/enriched.length*100,1):null,avgMaxDrawdownPct:dd.length?round(mean(dd)):null,regimesCovered:regimes.length,archetypesCovered:archetypes.length,regimeStabilityPct:stable.length?round(stable.filter(Boolean).length/stable.length*100,1):null,pValue:normalTwoSidedP(t)};
}
