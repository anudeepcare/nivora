import type {FeatureCandidate,FeatureContext,FeatureFamily,FeatureTransform,ResearchHorizon} from './domain';

const B=(family:FeatureFamily,theory:string,...metrics:string[])=>metrics.map(baseMetric=>({family,theory,baseMetric}));
const BASE=[
...
export const FEATURE_FAMILIES=[...new Set(BASE.map(x=>x.family))];
export const FEATURE_TRANSFORMS:FeatureTransform[]=['LEVEL','SLOPE','ACCELERATION','PERCENTILE','DIVERGENCE','CROSSOVER','ZSCORE','REGIME_NORMALIZED'];
export const RESEARCH_HORIZONS:ResearchHorizon[]=['1D','5D','20D','90D','180D','1Y'];
export const FEATURE_CONTEXTS:FeatureContext[]=['NONE','SECTOR_CONFIRM','MARKET_REGIME','VOLUME_CONFIRM','TREND_CONFIRM','VALUATION_CONFIRM','EARNINGS_CONFIRM','RISK_CONFIRM'];

export function generateFeatureCatalog(options:{limit?:number}={}):FeatureCandidate[]{
 const limit=Math.max(0,Math.floor(options.limit??Number.MAX_SAFE_INTEGER));
 const out:FeatureCandidate[]=[];
 outer: for(const base of BASE) for(const transform of FEATURE_TRANSFORMS) for(const horizon of RESEARCH_HORIZONS) for(const context of FEATURE_CONTEXTS){
...
export function featureCatalogSummary(){
 const catalog=generateFeatureCatalog();
 const byFamily=Object.fromEntries(FEATURE_FAMILIES.map(f=>[f,catalog.filter(x=>x.family===f).length]));
 return{candidateCount:catalog.length,baseMetricCount:BASE.length,familyCount:FEATURE_FAMILIES.length,byFamily,theoryCount:new Set(catalog.map(x=>x.theory)).size};
}
