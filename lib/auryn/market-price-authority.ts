...
export type DisplayLabel="PRE-MARKET PRICE"|"LIVE MARKET PRICE"|"AFTER-HOURS PRICE"|"LAST MARKET PRICE"|"LAST OFFICIAL CLOSE"|"ACTIVE_SESSION_PRICE_UNAVAILABLE"|"PRICE VERIFYING";
...
export function selectMarketDisplayQuote(input:{
 ...
 if(!valid.length){
   ...
   if(input.session==="PRE_MARKET"||input.session==="AFTER_HOURS")return{symbol,price:null,session:input.session,label:"ACTIVE_SESSION_PRICE_UNAVAILABLE",asOf:null,source:null,freshness:"VERIFYING",confidence:"CONTESTED",providerAgreementPct:null,reason:"Active session price unavailable; no fresh session-appropriate quote or trade is available."};
