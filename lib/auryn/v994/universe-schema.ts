...
export function buildExchangeStratifiedUniverse(rows:MarketUniverseRow[],limit=300,seed=""){
 ...
 for(const a of groups.values())a.sort((x,y)=>hash(`${seed}:${x.symbol}`)-hash(`${seed}:${y.symbol}`));
 const keys=[...groups.keys()].sort((a,b)=>hash(`${seed}:${a}`)-hash(`${seed}:${b}`)),out:string[]=[];let progress=true;
