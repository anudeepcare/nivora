...
export default function CalibrationPage(){
 const[data,setData]=useState<any>(null),[modelHealth,setModelHealth]=useState<any>(null),[loading,setLoading]=useState(true);
 useEffect(()=>{Promise.all([fetch("/api/calibration",{cache:"no-store"}).then(r=>r.json()),fetch("/api/model-health",{cache:"no-store"}).then(r=>r.json()).catch(()=>null)]).then(([c,m])=>{setData(c);setModelHealth(m)}).finally(()=>setLoading(false))},[]);
 ...
 return <AppShell><section className="v65EvidencePage">
  <div className="v65PageHead"><div><div className="eyebrow">MODEL EVIDENCE</div><h1>Evidence before confidence.</h1><p>Backtests, exact-engine outcomes and paper results are separate. AURYN V6 never calls zero observations “0% accuracy” and never silently rewrites production weights.</p></div></div>

  {loading?<div className="tradingLabEmpty">Loading model evidence…</div>:<>
   <div className="v65EvidenceState"><article><small>MODEL PROOF GRADE</small><b>{modelHealth?.proof?.grade||"UNPROVEN"}</b><span>N={modelHealth?.proof?.exactSampleN??0} exact-engine matured outcomes</span></article><article><small>ACTION LADDER</small><b>{modelHealth?.proof?.actionLadder?.monotonic?"MONOTONIC":"NOT PROVEN"}</b><span>Strong Buy → Buy → Hold → Reduce → Sell must separate by alpha.</span></article><article><small>PROMOTION GATE</small><b>{modelHealth?.proof?.promotion?.eligible?"ELIGIBLE":"BLOCKED"}</b><span>Auto promotion remains OFF.</span></article><article><small>CONFIDENCE DISCIPLINE</small><b>SEPARATE</b><span>Evidence confidence is not a probability of profit.</span></article></div>
   ...
   {Number(data?.learning?.maturedOutcomes||0)===0?<div className="v65NoEvidence"><small>NO MEASURED RESULT YET</small><h2>Collection is working, proof has not matured.</h2><p>AURYN records frozen exact-engine decisions now. The scheduled maturity workflow later measures 30D, 90D, 180D, 1Y and 2Y results against the benchmark. Until those dates arrive, hit rate, alpha, Brier and ECE stay blank instead of showing fake zeros.</p><div><span><b>1</b> Decision snapshot</span><span><b>2</b> Outcome matures</span><span><b>3</b> Benchmark comparison</span><span><b>4</b> Challenger evaluation</span></div></div>:<>
    <div className="v65EvidenceMetrics">
     <article><small>90D EXACT-ENGINE SAMPLE <MetricInfo title="Exact-engine sample">Number of matured 90-day outcomes produced by this exact engine cohort.</MetricInfo></small><b>{exact.n??0}</b><span>Minimum {exact.minimum??30} for calibrated status</span></article>
     ...
     <article><small>WEIGHTS-COMPATIBLE SAMPLE <MetricInfo title="Weights-compatible sample">Older decisions using the same thesis-weight contract. This evidence is pooled separately and never presented as exact V6 proof.</MetricInfo></small><b>{s.n??0}</b><span>Pooled historical evidence shown separately</span></article>
    </div>

    <section className="tradingAudit"><div className="tradingAuditHead"><div><small>HORIZON EVIDENCE</small><h2>Pooled history vs exact engine cohort</h2></div></div>
