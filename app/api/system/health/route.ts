...
export async function GET(){
 ...
 return NextResponse.json({status:configured>=4?"operational":"degraded",architecture:"NIVORA V59",configured,expected:Object.keys(env).length,capabilities:env,redundancy,providers:providers.map(p=>({id:p.id,capabilities:p.capabilities,realTime:p.realTime})),note:"A capability marked non-redundant remains a provider single point of failure; V59 exposes this rather than implying failover exists."});
}
