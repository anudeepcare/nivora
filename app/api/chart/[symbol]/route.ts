...
export async function GET(req:Request,{params}:{params:Promise<{symbol:string}>}){
 ...
 const hint=providerMarketHint(symbol),url=`https://api.twelvedata.com/time_series?symbol=${encodeURIComponent(symbol)}&interval=${cfg.interval}&outputsize=${cfg.size}&timezone=UTC${(range==="1D"||range==="1W")?"&prepost=true":""}${hint.exchange?`&exchange=${encodeURIComponent(hint.exchange)}`:""}&apikey=${key}`;
