...
async function series(symbol:string,key:string,size=280,revalidate=300){
...
export async function GET(_:Request,{params}:{params:Promise<{symbol:string}>}){
 ...
 try{
  ...
  const [j,bj]=await Promise.all([series(symbol,key),benchmark?series(benchmark,key,120,900).catch(()=>null):Promise.resolve(null)]);
  ...
  let bench20=0,bench60=0,marketTrend=50;
  if(bj?.values){const br=bj.values.slice().reverse(),bc=br.map((x:any)=>+x.close),bp=bc.at(-1)!;bench20=bp/(bc.at(-21)??bp)-1;bench60=bp/(bc.at(-61)??bp)-1;const be20=ema(bc,20).at(-1)!,be50=ema(bc,50).at(-1)!;marketTrend=clamp(50+(bp>be20?18:-18)+(be20>be50?18:-18)+bench20*80,12,88)}
