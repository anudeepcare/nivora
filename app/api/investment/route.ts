...
export async function GET(req:Request){
  ...
  const items=(data||[]).map((x:any)=>({symbol:x.symbol,storedPrice:Number(x.price||0),priceRole:"STORED_SCAN_CONTEXT",changePct:Number(x.change_pct||0),storedAction:x.action,actionRole:"STORED_SCAN_CONTEXT",reason:x.reason,thesisScore:Number(x.thesis_score||0),thesisState:x.thesis_state,opportunityScore:Number(x.opportunity_score||0),confidence:Number(x.evidence_confidence||0)}));
