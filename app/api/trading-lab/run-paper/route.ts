...
async function run(req:Request,automatic=false){
 ...
 for(const s of latest.values()){
  ...
  let quote:ExecutionQuote;try{const aq=await broker.getLatestExecutionQuote(s.symbol);quote=normalizeAlpacaQuote(s.symbol,aq.quote,aq.trade,new Date())}catch{const raw=await sharedJson(`https://api.twelvedata.com/quote?symbol=${encodeURIComponent(s.symbol)}&prepost=true&apikey=${twelve}`,["twelve","trade-lab-quote",s.symbol],5,1800);const tq=normalizeTwelveQuote(raw,new Date());quote={...tq,bid:null,ask:null,spreadPct:null,provider:"twelvedata"}}
  const context={equity:account.equity,cash:account.cash,dailyPnlPct:account.dailyPnlPct,currentPositionValue:Math.abs(pos?.marketValue||0),openPositions:positions.length,duplicate:false,quote:{price:quote.price,ageSeconds:quote.ageSeconds,freshness:quote.freshness,changePct:"changePct" in quote?quote.changePct:null,spreadPct:quote.spreadPct}};
