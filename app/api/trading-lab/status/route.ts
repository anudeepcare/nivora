...
export async function GET(){
 ...
 const [{data:trades},{count:intents},{count:orders},{count:snapshots},{count:authorized},{count:blocked}]=await Promise.all([
  ...
  db.from("nivora_v61_trade_intents").select("id",{count:"exact",head:true}).eq("status","AUTHORIZED"),
  db.from("nivora_v61_trade_intents").select("id",{count:"exact",head:true}).eq("status","BLOCKED")
 ]);
 ...
 return NextResponse.json({status:configured?"ready":"paper-broker-not-configured",version:TRADING_LAB_VERSION,engineVersion:ENGINE_VERSION,broker:{name:"Alpaca",mode:"paper",configured},liveExecution:"approval-required",intents:intents||0,orders:orders||0,funnel:{snapshots:snapshots||0,evaluated:snapshots||0,intents:intents||0,authorized:authorized||0,blocked:blocked||0,submitted:orders||0},metrics,note:"Trading Lab metrics are realized paper results, not a guarantee of future returns."},{headers:{"Cache-Control":"private, max-age=30"}});
}
