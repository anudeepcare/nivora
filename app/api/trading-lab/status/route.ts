...
export async function GET(){
 ...
 const recentEvaluations=latest.slice(0,50).map(e=>{const order=e.client_order_id?orderByClient.get(String(e.client_order_id)):null;const fill=order?fillByOrder.get(Number(order.id)):null;return{symbol:e.symbol,action:e.today_action||"NONE",status:e.status,reason:e.reason,riskCode:e.risk_code||null,evaluatedAt:e.evaluated_at,quoteProvider:e.details?.quoteProvider||null,quoteAgeSeconds:e.details?.quoteAgeSeconds??null,marketSession:e.details?.marketSession||null,automatic:Boolean(e.details?.automatic),clientOrderId:e.client_order_id||null,orderStatus:order?.status||null,submittedAt:order?.submitted_at||null,fillStatus:fill?"FILLED":null,filledAt:fill?.filled_at||null,fillPrice:fill?.fill_price==null?null:Number(fill.fill_price),realizedPnl:fill?.realized_pnl==null?null:Number(fill.realized_pnl),returnPct:fill?.return_pct==null?null:Number(fill.return_pct)}});
 ...
 const latestAutomatic=evaluations.find(e=>Boolean(e.details?.automatic));const currentSession=(await import("@/lib/nivora-market-session")).marketSessionAt(new Date());
 return NextResponse.json({status:configured?"ready":"paper-broker-not-configured",version:TRADING_LAB_VERSION,engineVersion:ENGINE_VERSION,broker:{name:"Alpaca",mode:"paper",configured},runner:{automatic:true,schedule:"Every 5 minutes, weekdays",marketSession:currentSession,lastAutomaticRun:latestAutomatic?.evaluated_at||null},liveExecution:"approval-required",auditStatus,auditReason:evaluationsResult.error?.message||null,intents:intentsResult.count||0,orders:ordersResult.count||0,funnel:{snapshots:snapshotsResult.count||0,evaluated:uniqueSnapshots.size,intents:intentCount,authorized:has("SUBMITTED"),blocked:has("BLOCKED"),submitted:has("SUBMITTED")},recentEvaluations,metrics,note:"Trading Lab metrics are realized paper results, not a guarantee of future returns."},{headers:{"Cache-Control":"private, max-age=15"}});
}
