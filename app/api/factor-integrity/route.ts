...
export async function GET(){
 ...
 const{data}=await db.from("nivora_decision_history").select("company_score,growth_score,financial_score,analyst_score,valuation_score,thesis_score,opportunity_score,engine_version").eq("engine_version","v57").limit(5000);
 ...
 return NextResponse.json({status:rows.length>=50?"active":"collecting",n:rows.length,correlations,uniqueInformationBudget:uniqueInformationBudget(correlations),note:"V57 monitors factor-family correlation before applying any orthogonalization. Correlation findings are diagnostic and do not automatically change ratings."},{headers:{"Cache-Control":"private, max-age=900"}})
}
