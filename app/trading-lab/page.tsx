...
export default function TradingLab(){
 ...
 return <AppShell><section className="tradingLabPage">
  ...
  <div className="tradingLabSafety"><ShieldCheck size={18}/><div><b>No live-money auto execution</b><span>Autonomous execution is restricted to the configured paper account. Any live broker path remains approval-required.</span></div></div><div className="tradingLabMethod"><Activity size={18}/><div><b>AUTOMATIC RUNNER</b><p>{data?.runner?.automatic?"ON":"OFF"} · {data?.runner?.schedule||"—"} · Market: {data?.runner?.marketSession||"—"} · Last auto evaluation: {when(data?.runner?.lastAutomaticRun)}</p></div><TrendingUp size={18}/></div>
