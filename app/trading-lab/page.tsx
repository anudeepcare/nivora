...
export default function TradingLab(){
 ...
 return <AppShell><section className="aurynLabPage">
  ...
  {loading?<div className="aurynLabEmpty">Loading paper account…</div>:<>
   <section className="aurynLabPath"><div className="aurynLabSectionHead aurynLabPathHead"><div><small>PAPER PATH</small><h2>From decision to proof</h2></div><span>Each step narrows the funnel. A blocked trade is evidence too—it tells you exactly which safety gate stopped execution.</span></div><div className="aurynLabFunnel"><article><small>DECISIONS CHECKED</small><b>{evaluated.toLocaleString()}</b><span>fresh decisions evaluated</span></article><article className={actionable?"good":"mid"}><small>QUALIFIED NOW · BUY SIGNALS</small><b>{actionable.toLocaleString()}</b><span>qualified at decision layer</span></article><article className={Number(data?.funnel?.blocked||0)?"mid":"good"}><small>RISK / EXECUTION</small><b>{Number(data?.funnel?.blocked||0).toLocaleString()}</b><span>blocked before broker submission</span></article><article className={orders?"good":"mid"}><small>PAPER ORDERS</small><b>{orders.toLocaleString()}</b><span>{orders?"submitted to Alpaca Paper":"none submitted yet"}</span></article><article className={trades?"good":"mid"}><small>COMPLETED</small><b>{trades.toLocaleString()}</b><span>{trades?`${m.winRatePct}% paper win rate`:"waiting for round trips"}</span></article></div></section>
