...
function PortfolioContent(){
 ...
 return <section className="portfolioPage v65Portfolio v653Portfolio">
  ...
  <div id="portfolio-holdings" className="v653HoldingsHead"><div><small>POSITIONS</small><h2>Your holdings</h2><em>{investedRows.length.toLocaleString()} investments</em></div><span>Sorted as tracked · open any name for the full decision.</span></div>
  <div className="v65PositionList">{rows.length?rows.map((x:any)=>{
   const isCash=x.asset_type==="CASH",q=quotes[x.symbol],price=isCash?1:Number(q?.price||x.avg_cost||0),mv=Number(x.shares||0)*price,ret=isCash?0:(x.avg_cost?((price/x.avg_cost)-1)*100:null),isEdit=edit?.id===x.id,action=isCash?"LIQUIDITY":String(q?.action||"REVIEW"),hasThesis=Number.isFinite(Number(q?.thesisScore)),hasOpportunity=Number.isFinite(Number(q?.opportunityScore));
   return <article className="v65Position v653Position" key={x.id}>{isEdit?<div className="positionEdit"><b>{x.symbol}</b><input type="number" step="any" value={edit.shares} onChange={e=>setEdit({...edit,shares:e.target.value})}/>{!isCash?<input type="number" step="0.01" value={edit.avg_cost} onChange={e=>setEdit({...edit,avg_cost:e.target.value})}/>:null}<button onClick={saveEdit}><Check size={16}/></button><button onClick={()=>setEdit(null)}><X size={16}/></button></div>:<><div className="v65PosMain">{isCash?<Banknote size={18}/>:x.asset_type==="CRYPTO"?<Bitcoin size={18}/>:<WalletCards size={18}/>}<div className="v6522HoldingSummary"><b>{x.symbol}</b><span className="v6523HoldingMeta">{x.asset_type}{isCash?` · $${Number(x.shares).toLocaleString()}`:` · ${Number(x.shares).toLocaleString(undefined,{maximumFractionDigits:4})} shares · Avg $${Number(x.avg_cost).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}`}</span></div></div><div className="v65PosMetrics"><span><small>VALUE</small><b className="v6523PrimaryValue">${mv.toLocaleString(undefined,{maximumFractionDigits:0})}</b><em>{intel.totalValue?`${(mv/intel.totalValue*100).toFixed(1)}% of portfolio`:""}</em></span><span><small>{isCash?"ALLOCATION":"RETURN"}</small><b className={`v6523PrimaryValue ${!isCash&&ret!=null&&ret<0?"bad":"good"}`}>{isCash?`${intel.totalValue?(mv/intel.totalValue*100).toFixed(1):0}%`:ret!=null?`${ret>=0?"+":""}${ret.toFixed(1)}%`:"—"}</b></span>{!isCash?<span className="v6523Current"><small>CURRENT</small><b>{`$${price.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}`}</b></span>:null}</div><div className="v6522DecisionRow"><div><b className={/BUY|ADD|HOLD/.test(action)?"good":/AVOID|EXIT|TRIM|SELL/.test(action)?"bad":"mid"}>{action.replaceAll("_"," ")}</b>{!isCash&&(hasThesis||hasOpportunity)&&<em>{hasThesis?<i>THESIS {Math.round(Number(q.thesisScore))}</i>:null}{hasOpportunity?<i>OPPORTUNITY {Math.round(Number(q.opportunityScore))}</i>:null}</em>}</div>{!isCash?<Link href={`/stock/${encodeURIComponent(x.symbol)}`}>Decision →</Link>:null}</div><div className="v6526RowActions"><button type="button" aria-label={`Edit ${x.symbol}`} title="Edit" onClick={()=>setEdit({...x})}><Pencil size={16}/></button><button type="button" className="danger" aria-label={`Delete ${x.symbol}`} title="Delete" onClick={()=>remove(x)}><Trash2 size={16}/></button></div></>}</article>
  }):<div className="emptyToday"><Search size={22}/><b>No assets yet</b><span>Add stocks, crypto or cash.</span></div>}</div>

  <section className="aurynPortfolioDeep" id="portfolio-allocation"><header><small>ALLOCATION & RISK</small><h2>Where portfolio risk actually lives</h2></header><div className="v653PortfolioDetailsBody"><div className="v65Allocation"><div><small>ALLOCATION</small><h2>Where the money is</h2></div>{(["EQUITY","CRYPTO","CASH"] as const).map(k=><article key={k}><span>{k}</span><b>${intel.assetAllocation[k].toLocaleString(undefined,{maximumFractionDigits:0})}</b><em>{intel.totalValue?`${(intel.assetAllocation[k]/intel.totalValue*100).toFixed(1)}%`:"0%"}</em></article>)}</div>{portfolioRisk?<div id="portfolio-risk-detail" className="v65RiskOverlay"><ShieldCheck size={18}/><div><small>PORTFOLIO RISK</small><b>{portfolioRisk.riskLabel}</b><span>{portfolioRisk.notes?.[0]||"No major concentration warning."}</span></div><span>Largest {portfolioRisk.largestPositionPct}% · sector/archetype {portfolioRisk.largestSectorPct}%</span></div>:null}</div></section>
