...
function PortfolioContent(){
 ...
 return <section className="portfolioPage v65Portfolio v653Portfolio">
  ...
  <div className="v65PositionList">{rows.length?rows.map((x:any)=>{
   ...
   return <article className="v65Position v653Position" key={x.id}>{isEdit?<div className="positionEdit"><b>{x.symbol}</b><input type="number" step="any" value={edit.shares} onChange={e=>setEdit({...edit,shares:e.target.value})}/>{!isCash?<input type="number" step="0.01" value={edit.avg_cost} onChange={e=>setEdit({...edit,avg_cost:e.target.value})}/>:null}<button onClick={saveEdit}><Check size={16}/></button><button onClick={()=>setEdit(null)}><X size={16}/></button></div>:<><div className="v65PosMain">{isCash?<Banknote size={18}/>:x.asset_type==="CRYPTO"?<Bitcoin size={18}/>:<WalletCards size={18}/>}<div className="v6522HoldingSummary"><b>{x.symbol}</b><span>{x.asset_type}{isCash?` · $${Number(x.shares).toLocaleString()}`:` · avg $${Number(x.avg_cost).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}`}</span></div></div><div className="v65PosMetrics">{!isCash?<span><small>Qty</small><b>{Number(x.shares).toLocaleString(undefined,{maximumFractionDigits:4})}</b></span>:null}<span><small>VALUE</small><b>${mv.toLocaleString(undefined,{maximumFractionDigits:0})}</b></span><span><small>{isCash?"ROLE":"CURRENT"}</small><b>{isCash?"CASH":`$${price.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}`}</b></span><span><small>{isCash?"ALLOCATION":"RETURN"}</small><b className={!isCash&&ret!=null&&ret<0?"bad":"good"}>{isCash?`${intel.totalValue?(mv/intel.totalValue*100).toFixed(1):0}%`:ret!=null?`${ret>=0?"+":""}${ret.toFixed(1)}%`:"—"}</b></span></div><div className="v6522DecisionRow"><div><small>NIVORA</small><b className={/BUY|ADD|HOLD/.test(action)?"good":/AVOID|EXIT|TRIM|SELL/.test(action)?"bad":"mid"}>{action.replaceAll("_"," ")}</b>{!isCash&&<em><i>THESIS {Number.isFinite(Number(q?.thesisScore))?Math.round(Number(q.thesisScore)):"—"}</i><i>OPPORTUNITY {Number.isFinite(Number(q?.opportunityScore))?Math.round(Number(q.opportunityScore)):"—"}</i></em>}</div>{!isCash?<Link href={`/stock/${encodeURIComponent(x.symbol)}`}>Decision →</Link>:null}</div><div className="rowActions v6522HoldingActions"><button onClick={()=>setEdit({...x})} title="Edit" aria-label={`Edit ${x.symbol}`}><Pencil size={16}/></button><button className="danger" onClick={()=>remove(x)} title="Delete" aria-label={`Delete ${x.symbol}`}><Trash2 size={16}/></button></div></>}</article>
