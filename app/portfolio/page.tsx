...
function PortfolioContent(){
 ...
 return <section className="portfolioPage v65Portfolio v653Portfolio">
  ...
  <div className="v653PortfolioActions">
   <article className="good"><small>STRONGEST THESIS</small><b>{strongest?.symbol||"—"}</b><span>{strongest?`${Number(strongest.q.thesisScore).toFixed(0)}/100 · ${String(strongest.q.action||"Review").replaceAll("_"," ")}`:"Analyze holdings to populate thesis evidence."}</span>{strongest?<Link href={`/stock/${encodeURIComponent(strongest.symbol)}`}>Open →</Link>:null}</article>
   <article><small>BEST NEW-MONEY SETUP</small><b>{opportunity?.symbol||"—"}</b><span>{opportunity?`${Number(opportunity.q.opportunityScore).toFixed(0)}/100 opportunity · ${String(opportunity.q.action||"Review").replaceAll("_"," ")}`:"No scored opportunity yet."}</span>{opportunity?<Link href={`/stock/${encodeURIComponent(opportunity.symbol)}`}>Open →</Link>:null}</article>
   <article className="bad"><small>REVIEW FIRST</small><b>{weakest?.symbol||"—"}</b><span>{weakest?`${Number(weakest.q.thesisScore).toFixed(0)}/100 thesis · ${String(weakest.q.action||"Review").replaceAll("_"," ")}`:"No weak thesis identified yet."}</span>{weakest?<Link href={`/stock/${encodeURIComponent(weakest.symbol)}`}>Open →</Link>:null}</article>
  </div>

  <details className="v653AddAsset"><summary>+ Add stock, crypto or cash</summary><div className="v65AddAsset">
  ...
  </div></details>{msg&&<div className="portfolioMsg">{msg}</div>}
