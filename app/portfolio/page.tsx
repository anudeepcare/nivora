...
import {aggregatePortfolioLots} from "@/lib/auryn/portfolio-lots";
...
function PortfolioContent(){
 ...
 const[pulseHistory,setPulseHistory]=useState<any[]>([]),[assetType,setAssetType]=useState<AssetType>("EQUITY"),[symbol,setSymbol]=useState(sp.get("symbol")||""),[shares,setShares]=useState(""),[cost,setCost]=useState(""),[accountName,setAccountName]=useState("Default"),[horizon,setHorizon]=useState("long"),[msg,setMsg]=useState(""),[edit,setEdit]=useState<any>(null),[portfolioRisk,setPortfolioRisk]=useState<any>(null),[showAdd,setShowAdd]=useState(false),[pendingDelete,setPendingDelete]=useState<any>(null);
 ...
 const load=useCallback(async()=>{
  ...
  const cashMap=new Map<string,any>(),sourceRows:any[]=[];for(const x of normalized){if(x.asset_type==="CASH"){const k=String(x.currency||x.symbol||"USD").toUpperCase(),prev=cashMap.get(k);if(prev)prev.shares=Number(prev.shares||0)+Number(x.shares||0);else cashMap.set(k,{...x,currency:k,symbol:k,account_name:x.account_name||"Default"})}else sourceRows.push({...x,account_name:x.account_name||"Default"})}
  const clean:any[]=[...aggregatePortfolioLots(sourceRows),...cashMap.values()];setRows(clean);
 ...
 async function add(e:React.FormEvent){
  ...
  const account=(accountName.trim()||"Default");
  const payload={user_id:user.id,account_name:account,symbol:normalized,shares:qty,avg_cost:avg,horizon,asset_type:assetType,currency:assetType==="CASH"?normalized.split("/")[0]:"USD",updated_at:new Date().toISOString()};
  const{error}=await s.from("portfolio_positions").upsert(payload,{onConflict:"user_id,account_name,symbol"});
 ...
 async function saveEdit(){if(!edit)return;const s=supabaseBrowser();const{error}=await s.from("portfolio_positions").update({shares:+edit.shares,avg_cost:edit.asset_type==="CASH"?1:+edit.avg_cost,account_name:String(edit.account_name||"Default").trim()||"Default",horizon:edit.horizon,updated_at:new Date().toISOString()}).eq("id",edit.id);if(error){setMsg(error.message);return}setEdit(null);setMsg("Position updated");load()}
 ...
 const priced=useMemo(()=>rows.map((x:any)=>{
  ...
  return{assetType:x.asset_type==="CRYPTO"?"CRYPTO" as const:"EQUITY" as const,symbol:x.symbol,quantity:Number(x.shares||0),price,avgCost:Number(x.avg_cost||0),accountLots:x.accountLots||[{...x,account_name:x.account_name||"Default"}],thesisScore:q?.thesisScore??null,companyScore:q?.companyScore??null,opportunityScore:q?.opportunityScore??null,action:portfolioOwnerAction(q),sector:q?.sector||null,archetype:q?.archetype||null,marketIntelligence:q?.marketIntelligence??null,levels:q?.levels??null,source:x};
 }),[rows,quotes]);
 ...
 return <section className="aurynPortfolioPage">
  ...
  {showAdd?<div className="aurynSection aurynAddPanel">
   ...
   <form className="aurynAssetForm" onSubmit={add}><input placeholder={assetType==="CASH"?"Currency (USD)":assetType==="CRYPTO"?"BTC, ETH, SOL…":"Ticker"} value={symbol} onChange={e=>setSymbol(e.target.value)} required={assetType!=="CASH"}/><input placeholder={assetType==="CASH"?"Cash amount":"Qty"} type="number" step="any" value={shares} onChange={e=>setShares(e.target.value)} required/>{assetType!=="CASH"?<input placeholder="Average cost" type="number" step="0.01" value={cost} onChange={e=>setCost(e.target.value)} required/>:null}<input aria-label="Account / Broker" placeholder="Account / Broker (Robinhood, Moomoo…)" value={accountName} onChange={e=>setAccountName(e.target.value)}/>{assetType!=="CASH"?<select value={horizon} onChange={e=>setHorizon(e.target.value)}><option value="short">Short term</option><option value="swing">Swing</option><option value="long">Long term</option></select>:null}<button>Add {assetType==="EQUITY"?"stock":assetType==="CRYPTO"?"crypto":"cash"}</button></form>
