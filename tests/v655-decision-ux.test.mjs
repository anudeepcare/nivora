...
test("metric help uses one glyph and viewport-safe positioning",()=>{const m=read("components/v65/MetricInfo.tsx"),c=read("app/globals.css");assert.match(m,/Info className="v654InfoGlyph"/);assert.match(c,/\.v655MetricSheet/);assert.match(c,/position:fixed/);});
test("portfolio removes redundant start-here heading",()=>{const s=read("app/portfolio/page.tsx");assert.doesNotMatch(s,/>Start here\.</);assert.match(s,/Portfolio priorities/);});
