...
test("info affordance is visibly circled and keeps large hit target",()=>{assert.match(css,/\.v658InfoButton\{[^}]*border:1px solid[^}]*border-radius:50%/s);assert.match(css,/\.v658InfoButton::after\{[^}]*inset:-11px/s);assert.match(info,/v6516InfoGlyph/)});
