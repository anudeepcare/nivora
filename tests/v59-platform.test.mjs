...
test("V61 Trading Lab preserves V59 scoring weights",()=>{assert.equal(ENGINE_VERSION,"v64");assert.equal(WEIGHTS_VERSION,"v59-thesis-1");assert.equal(TODAY_POLICY_VERSION,"v64-proof-aware-today-1")});

test("Today policy cannot bypass a broken thesis",()=>{const t=deriveTodayAction({thesisScore:85,opportunityScore:90,companyScore:82,thesisLabel:"BULLISH",thesisState:"Broken",timing:{label:"ATTRACTIVE",score:80},valuationLabel:"Attractive",vetoes:[],consistency:{ok:true,notes:[]}},false);assert.equal(t.action,"WAIT");assert.equal(t.blocked,true)});
