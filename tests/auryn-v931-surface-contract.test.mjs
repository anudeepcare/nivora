...
test('stock-wide institutional brief sits above evidence tabs so every stock tab shares one call',()=>{
 ...
 assert.ok(s.indexOf('<AurynResearchOverview')<s.indexOf('<StockEvidenceNav'));
 ...
 assert.match(brief,/PREFERRED ENTRY|Preferred Entry/);
});

test('stock experience has one canonical expert view with no legacy decision duplicate',()=>{
 const stock=read('components/StockClient.tsx');
 const brief=read('components/premium/AurynResearchOverview.tsx');
 assert.match(stock,/AurynResearchOverview decision=\{institutionalDecision\} marketTruth=\{marketTruth\}/);
