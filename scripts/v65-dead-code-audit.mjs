...
for(const f of fs.readdirSync(".")){if(/^README_V\d|^V\d.*(?:RELEASE|AUDIT|ARCHITECTURE).*\.md$/i.test(f))problems.push(`${f}: legacy release document left in root`)}
for(const f of ["tsconfig.tsbuildinfo",".next",".engine-test"]){if(fs.existsSync(f))problems.push(`${f}: generated build artifact should not ship`)}
