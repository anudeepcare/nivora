...
const run=(cmd,args)=>{console.log(`\n[v9.3.4.1] ${cmd} ${args.join(' ')}`);const r=spawnSync(cmd,args,{stdio:'inherit',shell:false,env:process.env});if(r.status!==0)process.exit(r.status??1)};
...
if(base){run('npm',['run','audit:v934-live','--',`--limit=${liveLimit}`]);console.log('\nAURYN V9.3.4.1 RELEASE GATE: PASS · CODE + REAL-WORLD MARKET INTELLIGENCE AUDIT');}
else if(requireLive){console.error('\nAURYN V9.3.4.1 RELEASE GATE: BLOCKED · AURYN_BASE_URL is required for real-world validation.');process.exit(2);}
else console.log('\nAURYN V9.3.4.1 CODE GATE: PASS · CODE_READY_LIVE_VALIDATION_REQUIRED');
