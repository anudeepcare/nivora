const BASE=(process.env.AURYN_BASE_URL||"https://getauryn.vercel.app").replace(/\/$/,""),SECRET=process.env.CRON_SECRET||"",MAX_WORKERS=Math.max(1,Math.min(10,Number(process.env.AURYN_QUEUE_BURST||10)));
...
let processed=0;
for(let i=0;i<MAX_WORKERS;i++){const x=await call("/api/validation/worker");console.log("worker",x);if(x.status==="idle"||x.status==="deferred")break;
if(x.status==="skipped"){console.log("worker skipped",x);continue;}
processed+=Number(x.saved||0);await new Promise(r=>setTimeout(r,6000));}
