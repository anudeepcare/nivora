...
exports.evaluateBuyCalibration = evaluateBuyCalibration;
exports.auditDecisionDistribution = auditDecisionDistribution;
const n = (x, f = 50) => Number.isFinite(Number(x)) ? Number(x) : f;
const yes = (ok, label, passed, failed, gap = 1) => {
    (ok ? passed : failed).push(label);
    return ok ? 0 : gap;
};
function hardBlocks(x) {
    const out = [];
    if (x.consistency?.ok === false)
        out.push(`Consistency gate: ${x.consistency?.notes?.[0] || "decision evidence conflicts."}`);
    if ((x.vetoes?.length || 0) > 0)
        out.push(`Hard veto: ${x.vetoes[0]}`);
    if (x.thesisState === "Broken" || x.thesisScore < 29)
        out.push("Long-term thesis is broken or below the minimum capital threshold.");
    if (String(x.thesisLabel).toUpperCase() === "BEARISH")
        out.push("Bearish fundamental thesis blocks new capital.");
    const timing = String(x.timing?.label || "WAIT").toUpperCase();
    if (timing === "OVEREXTENDED")
        out.push("Timing is OVEREXTENDED; NIVORA will not chase.");
    if (String(x.stabilizationState || "").toUpperCase() === "REQUIRED")
        out.push("Price stabilization is required before new capital.");
    if (String(x.earlyWarningLevel || "").toUpperCase() === "HIGH")
        out.push("Fast-moving early-warning risk is HIGH; new capital is withheld.");
    if (String(x.marketModelDisagreement || "").toUpperCase() === "HIGH" && String(x.valuationRobustness || "").toUpperCase() === "FRAGILE")
        out.push("High market/model disagreement plus fragile valuation blocks new capital.");
    return out;
}
function qualityCompounder(x) {
    const p = [], f = [];
    let d = 0;
    ...
    if (String(x.stabilizationState || "").toUpperCase() === "WATCH")
        d += yes(timing >= 55, "WATCH stabilization requires timing ≥ 55", p, f, Math.max(0, 55 - timing) * 1.5);
...
function cyclicalValue(x) {
    ...
    if (x.valuationAvailable === true)
        d += yes(x.valuationLabel !== "Expensive", "Valuation is not Expensive", p, f, x.valuationLabel === "Expensive" ? 8 : 0);
    const eligible = f.length === 0;
...
function highConvictionStarter(x) {
    const p = [], f = [];
    let d = 0;
    const timing = n(x.timing?.score, 0), financial = n(x.factors?.financial), growth = n(x.factors?.growth), forward = n(x.factors?.forward), risk = n(x.factors?.risk, 60);
    ...
    if (growthHeavy)
        d += yes(growth >= 80, "Growth evidence ≥ 80", p, f, Math.max(0, 80 - growth));
...
function candidatePaths(x) {
    const a = String(x.archetype || "general").toLowerCase(), starter = x.thesisScore >= 80 ? [highConvictionStarter(x)] : [];
    if (a === "compounder" || a === "infrastructure")
        return [qualityCompounder(x), ...starter, balanced(x)];
    if (a === "cyclical" || a === "miner")
        return [cyclicalValue(x), balanced(x), ...starter];
    if (a === "hypergrowth" || a === "ai_infrastructure")
        return [growthMomentum(x), ...starter, balanced(x)];
    if (a === "pre_scale")
        return [catalystGrowth(x), ...starter];
    if (a === "biotech")
        return [catalystGrowth(x)];
    if (a === "bank" || a === "insurer")
        return [financialValue(x), balanced(x), ...starter];
    return [balanced(x), qualityCompounder(x), ...starter];
}
function evaluateBuyCalibration(x) {
    const hard = hardBlocks(x);
    const paths = candidatePaths(x).sort((a, b) => a.distance - b.distance);
    if (hard.length)
        return { eligible: false, path: null, tier: null, blockers: hard, primaryBlocker: hard[0], closestPath: paths[0]?.path || null, paths, hardBlocked: true };
    const winner = paths.find(p => p.eligible);
    if (winner)
        return { eligible: true, path: winner.path, tier: winner.tier, blockers: [], primaryBlocker: "", closestPath: winner.path, paths, hardBlocked: false };
...
function auditDecisionDistribution(rows) {
    const actions = {}, paths = {}, blockers = {};
    const details = rows.map(x => {
        const cal = evaluateBuyCalibration(x);
        let action;
        if (cal.eligible)
            action = "BUY";
        else if ((x.vetoes?.length || 0) > 0 || x.consistency?.ok === false || x.thesisState === "Broken" || x.thesisScore < 29 || String(x.thesisLabel).toUpperCase() === "BEARISH")
            action = "AVOID";
        else if (String(x.thesisLabel).toUpperCase() === "BULLISH")
            action = "WAIT";
        else
            action = "NO ACTION";
        actions[action] = (actions[action] || 0) + 1;
        if (cal.path)
            paths[cal.path] = (paths[cal.path] || 0) + 1;
        if (!cal.eligible && cal.primaryBlocker)
            blockers[cal.primaryBlocker] = (blockers[cal.primaryBlocker] || 0) + 1;
