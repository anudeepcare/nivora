...
exports.replaySymbol = replaySymbol;
const nivora_technical_engine_1 = require("./nivora-technical-engine");
const nivora_backtest_fundamentals_1 = require("./nivora-backtest-fundamentals");
const nivora_investor_1 = require("./nivora-investor");
const clampIdx = (arr, asOfDate) => {
    // Bars must be ascending by datetime. Find the last index whose date <= asOfDate.
    let idx = -1;
    for (let i = 0; i < arr.length; i++) {
        if (arr[i].datetime <= asOfDate)
            idx = i;
        else
            break;
    }
 ...
 */
function replayOne(input, asOfDate, horizonDays) {
    const asOfIdx = clampIdx(input.archetypeHintBars, asOfDate);
    if (asOfIdx < 200)
        return null; // need real EMA200 history, matching the live route's minimum
    ...
    const technical = (0, nivora_technical_engine_1.computeTechnicalSnapshot)(rowsUpToDate, benchRowsUpToDate, input.benchmarkSymbol);
    if (!technical)
        return null;
    const fundamentals = (0, nivora_backtest_fundamentals_1.pointInTimeFundamentals)(input.companyFacts, asOfDate);
    ...
    const decision = (0, nivora_investor_1.buildInvestorDecision)({ market, company, context, owns: false });
    if (!decision)
        return null;
    ...
    if (exitIdx >= input.archetypeHintBars.length)
        return null; // not enough forward data yet — drop, don't extrapolate
...
function replaySymbol(input, asOfDates, horizonDays) {
    const out = [];
    for (const d of asOfDates) {
        const r = replayOne(input, d, horizonDays);
        if (r)
            out.push(r);
    }
    return out;
