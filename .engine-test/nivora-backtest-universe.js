...
exports.validateUniverseManifest = validateUniverseManifest;
function validateUniverseManifest(input) {
    const manifest = Array.isArray(input) ? input : (Array.isArray(input.symbols) ? input.symbols : []);
    ...
    const quality = survivorshipBiasControlled && missingCikPct <= 10 ? "DECISION_GRADE" : manifest.length ? "LIMITED" : "INVALID";
    const warnings = [];
    if (!pointInTimeCorrect)
        warnings.push("Universe membership is not proven point-in-time correct.");
    if (!includesDelisted)
        warnings.push("Delisted/removed securities are not explicitly included; survivorship bias may remain.");
    if (!delistingReturnsHandled)
        warnings.push("Delisting returns are not explicitly handled; failed securities can otherwise disappear before the outcome horizon.");
    if (missingCikPct > 10)
        warnings.push(`${missingCikPct.toFixed(1)}% of symbols are missing CIK identifiers, limiting point-in-time SEC reconstruction.`);
