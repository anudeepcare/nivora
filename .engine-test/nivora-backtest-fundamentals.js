...
exports.pointInTimeFundamentals = pointInTimeFundamentals;
...
function knownAsOf(facts, asOfDate, forms) {
...
function latestKnown(facts, tags, asOfDate) {
    const known = knownAsOf(factsFor(facts, tags), asOfDate, ["10-K", "10-Q", "20-F", "6-K", "40-F"]);
    return known.at(-1)?.val ?? null;
}
function annualSeriesKnown(facts, tags, asOfDate) {
    const known = knownAsOf(factsFor(facts, tags), asOfDate, ["10-K", "20-F", "40-F"]);
    const byEnd = new Map();
    for (const x of known)
        byEnd.set(String(x.end), x); // last-filed wins per period (handles restatements correctly: only restatements filed by asOfDate count)
    return [...byEnd.values()].sort((a, b) => String(a.end).localeCompare(String(b.end))).slice(-5);
}
function growthKnown(facts, tags, asOfDate) {
...
const score100 = (n) => Math.max(0, Math.min(100, Math.round(n)));
 ...
 * don't change).
 */
function pointInTimeFundamentals(companyFacts, asOfDate) {
    ...
    const revVals = history.map(x => x.revenue).filter((x) => x != null);
    const niVals = history.map(x => x.netIncome).filter((x) => x != null);
    ...
    let fiveScore = 50;
    if (revenueTrend === "Strong")
        fiveScore += 18;
    else if (revenueTrend === "Improving")
        fiveScore += 10;
    else if (revenueTrend === "Weakening")
        fiveScore -= 12;
    fiveScore += (profitableYears - .5) * 28;
    fiveScore += (positiveCashYears - .5) * 18;
    if (opMargin != null)
        fiveScore += Math.max(-10, Math.min(12, (opMargin - 8) * .7));
    if (leverage != null)
        fiveScore += leverage < 55 ? 8 : leverage > 85 ? -10 : 0;
    const fiveYearRecord = { score: score100(fiveScore), years: history.length, revenueTrend, history };
    let score = 0;
    if (revGrowth != null)
        score += revGrowth > 15 ? 2 : revGrowth > 0 ? 1 : -1;
    if (ni != null)
        score += +ni > 0 ? 1 : -1;
    if (fcf != null)
        score += fcf > 0 ? 1 : -1;
    if (opMargin != null && opMargin > 12)
        score += 1;
    if (leverage != null)
        score += leverage < 65 ? 1 : leverage > 85 ? -1 : 0;
    if (cash != null && liab != null && +cash > +liab * .25)
        score += 1;
