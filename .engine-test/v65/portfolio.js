...
exports.calculatePortfolioIntelligence = calculatePortfolioIntelligence;
exports.calculatePortfolioPulse = calculatePortfolioPulse;
exports.calculatePortfolioPeriod = calculatePortfolioPeriod;
const clamp = (x, a = 0, b = 100) => Math.max(a, Math.min(b, x));
function calculatePortfolioIntelligence(assets) {
    ...
    const cashValue = rows.filter((x) => x.assetType === "CASH").reduce((s, x) => s + x.value, 0);
    const investedValue = totalValue - cashValue;
    const assetAllocation = { EQUITY: 0, CRYPTO: 0, CASH: 0 };
    rows.forEach((x) => { const k = String(x.assetType); assetAllocation[k] += x.value; });
    const invested = rows.filter((x) => x.assetType !== "CASH");
    const weights = investedValue ? invested.map((x) => x.value / investedValue) : [];
    ...
    const scoreOf = (x) => x.thesisScore != null ? Number(x.thesisScore) : x.companyScore != null ? Number(x.companyScore) : null;
    const scorable = rows.filter((x) => x.assetType === "EQUITY" && scoreOf(x) != null && Number.isFinite(scoreOf(x)));
    const scorableHoldings = scorable.length;
    const thesisQuality = scorable.length ? scorable.reduce((s, x) => s + Number(scoreOf(x)), 0) / scorable.length : 50;
    const actionBurden = invested.length ? invested.filter((x) => /AVOID|SELL|TRIM|EXIT|WAIT/i.test(String(x.action || ""))).length / invested.length * 100 : 0;
...
function calculatePortfolioPulse(assets, snapshots = []) {
    const base = calculatePortfolioIntelligence(assets);
    const invested = assets.filter(x => x.assetType !== "CASH").map(x => {
    ...
    const sectors = {};
    for (const x of invested)
        if (x.assetType === "EQUITY" && String(x.sector || "").trim()) {
            const sector = String(x.sector).trim();
            sectors[sector] = (sectors[sector] || 0) + x.value;
        }
    ...
    const actions = invested.map(x => {
        const raw = String(x.action || "").toUpperCase();
        let portfolioAction = "HOLD", reason = "Thesis is being tracked.";
        const weight = base.totalValue ? x.value / base.totalValue * 100 : 0;
        if (/AVOID|EXIT|SELL/.test(raw)) {
            portfolioAction = "AVOID";
            reason = "Individual evidence is weak; review before adding capital.";
        }
        else if (weight >= 25) {
            portfolioAction = "TRIM_RISK";
            reason = `Position is ${weight.toFixed(1)}% of the portfolio; sizing risk is high even if the thesis remains intact.`;
        }
        else if (/WAIT|WATCH|TRIM/.test(raw)) {
            portfolioAction = "WATCH";
            reason = "Current AURYN evidence calls for patience or review.";
        }
        else if (/BUY|ADD|ACCUMULATE/.test(raw) && Number(x.opportunityScore || 0) >= 60) {
            portfolioAction = "ADD";
            reason = "Individual evidence is constructive and portfolio sizing is not excessive.";
        }
...
function periodTarget(period, now) {
    const d = new Date(now);
    d.setUTCHours(23, 59, 59, 999);
    if (period === "ALL")
        return null;
    if (period === "YTD")
        return new Date(Date.UTC(d.getUTCFullYear(), 0, 1, 23, 59, 59, 999));
    const n = { "1D": { days: 1 }, "1W": { days: 7 }, "1M": { months: 1 }, "3M": { months: 3 }, "6M": { months: 6 }, "1Y": { years: 1 }, "2Y": { years: 2 }, "3Y": { years: 3 }, "4Y": { years: 4 } };
    const x = n[period] || {};
    if (x.days)
        d.setUTCDate(d.getUTCDate() - x.days);
    if (x.months)
        d.setUTCMonth(d.getUTCMonth() - x.months);
    if (x.years)
        d.setUTCFullYear(d.getUTCFullYear() - x.years);
    return d;
}
function calculatePortfolioPeriod(history, period, now = new Date()) {
    ...
    const pct = (a, b) => a != null && b != null && Number(a) > 0 && Number(b) > 0 ? +((Number(b) / Number(a) - 1) * 100).toFixed(2) : null;
