...
function classifySecuritySymbol(raw) {
    ...
    if (!symbol)
        return { ...base, kind: "UNKNOWN", confidence: "LOW", supportedForEquityAnalysis: false, reason: "Empty security symbol." };
    if (symbol.includes("/"))
        return { ...base, kind: "CRYPTO", confidence: "HIGH", supportedForEquityAnalysis: false, reason: "Slash-delimited crypto pair." };
    ...
    if (/^[A-Z]{5}$/.test(symbol)) {
        ...
        if (suffix === "W")
            return { ...base, kind: "WARRANT", confidence: "HIGH", supportedForEquityAnalysis: false, reason: "Nasdaq fifth-letter warrant designator." };
        if (suffix === "R")
            return { ...base, kind: "RIGHT", confidence: "HIGH", supportedForEquityAnalysis: false, reason: "Nasdaq fifth-letter rights designator." };
        if (suffix === "U")
            return { ...base, kind: "UNIT", confidence: "HIGH", supportedForEquityAnalysis: false, reason: "Nasdaq fifth-letter unit designator." };
        if (suffix === "V")
            return { ...base, kind: "TEMPORARY", confidence: "HIGH", supportedForEquityAnalysis: false, reason: "Nasdaq fifth-letter temporary/when-issued designator." };
    }
