...
exports.assessActionLadder = assessActionLadder;
exports.evaluatePromotionGate = evaluatePromotionGate;
exports.summarizeModelProof = summarizeModelProof;
const ACTION_ORDER = ['STRONG_BUY', 'BUY', 'HOLD', 'REDUCE', 'SELL'];
const finite = (x) => typeof x === 'number' && Number.isFinite(x);
const round = (x, d = 2) => +x.toFixed(d);
function assessActionLadder(rows) {
    ...
    for (let i = 0; i < comparable.length - 1; i++) {
        ...
        comparablePairs++;
        if (Number(a.avgAlphaPct) < Number(b.avgAlphaPct))
            violations.push(`${a.action} alpha ${a.avgAlphaPct}% is below ${b.action} ${b.avgAlphaPct}%.`);
    }
...
function evaluatePromotionGate(input) {
    ...
    const blockers = [];
    if (!checks.exactSample)
        blockers.push(`Need at least 120 exact-engine matured outcomes; have ${input.exactSampleN}.`);
    if (!checks.positiveAlpha)
        blockers.push('Exact-engine benchmark-relative alpha is not positive.');
    if (!checks.drawdownControlled)
        blockers.push('Average max drawdown breaches the -20% promotion guardrail.');
    if (!checks.actionLadder)
        blockers.push('Decision action ladder is not monotonic enough for promotion.');
    if (!checks.regimeBreadth)
        blockers.push('Evidence must cover at least two market regimes.');
    if (!checks.horizonBreadth)
        blockers.push('No matured proof horizon is available.');
    return { eligible: Object.values(checks).every(Boolean), blockers, checks };
}
function summarizeModelProof(rows) {
