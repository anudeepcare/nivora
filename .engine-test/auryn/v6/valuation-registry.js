...
exports.resolveValuationMethod = resolveValuationMethod;
const methodFor = (m) => {
    if (['SAAS_SOFTWARE', 'MARKETPLACE_ADTECH', 'FINTECH_PAYMENTS', 'DIGITAL_HEALTH_PLATFORM'].includes(m))
        return 'GROWTH_EV_SALES_FCF';
    if (m === 'SEMICONDUCTOR_MEMORY_CYCLICAL')
        return 'CYCLE_NORMALIZED';
    if (['AI_DATA_CENTER_INFRA', 'POWER_UTILITY_INFRA', 'NETWORKING_COMPUTE_INFRA'].includes(m))
        return 'AI_INFRA_SOTP_CAPACITY';
    if (m === 'SPACE_SATELLITE')
        return 'FRONTIER_SCENARIO_RUNWAY';
    if (m === 'BANK' || m === 'INSURER')
        return 'BANK_PB_ROE_NIM';
    if (m === 'REIT')
        return 'REIT_FFO_AFFO';
    if (m === 'BIOTECH_PHARMA' || m === 'MEDTECH')
        return 'BIOTECH_SCENARIO_PIPELINE';
    if (m === 'ENERGY' || m === 'MINER_COMMODITY')
        return 'ENERGY_CYCLE_NAV';
    if (['GENERAL_COMPOUNDER', 'CONSUMER', 'INDUSTRIAL', 'SEMICONDUCTOR_DESIGNER', 'DEFENSE'].includes(m))
        return 'DCF_FCF_EARNINGS';
...
function resolveValuationMethod(classification, factor) {
    ...
    if (score == null)
        return { method, state: 'UNAVAILABLE', score: null, decisionGrade: false, requiredInputs: requirements[method], explanation: explanationFor(method) };
