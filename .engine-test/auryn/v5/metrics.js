...
exports.buildProfessionalMetrics = buildProfessionalMetrics;
const finite = (x) => typeof x === 'number' && Number.isFinite(x);
const avg = (a) => a.length ? a.reduce((x, y) => x + y, 0) / a.length : 0;
const ema = (a, n) => { if (!a.length)
    return null; const k = 2 / (n + 1); let x = a[0]; for (let i = 1; i < a.length; i++)
    x = a[i] * k + x * (1 - k); return x; };
const calcAdx = (bars, n = 14) => {
    if (bars.length < n * 2 + 1)
    ...
    const dx = [];
    let pdi = trS ? 100 * pS / trS : 0, mdi = trS ? 100 * mS / trS : 0;
    ...
    if (dx.length < n)
        return { adx: null, plusDI: pdi, minusDI: mdi };
    let adx = avg(dx.slice(0, n));
    for (let i = n; i < dx.length; i++)
        adx = ((adx * (n - 1)) + dx[i]) / n;
    return { adx, plusDI: pdi, minusDI: mdi };
};
const calcAdvanced = (bars) => {
    if (bars.length < 20)
        return null;
    const c = bars.map(x => Number(x.close)), h = bars.map(x => Number(x.high)), l = bars.map(x => Number(x.low)), v = bars.map(x => Number(x.volume || 0));
    const p = c.at(-1);
    ...
    const mean20 = avg(c20), sd20 = Math.sqrt(avg(c20.map(x => (x - mean20) ** 2)));
    const bollingerWidth = mean20 ? ((4 * sd20) / mean20) * 100 : null;
    ...
    const low14 = Math.min(...l.slice(-14)), high14 = Math.max(...h.slice(-14));
    const stochasticK = high14 > low14 ? ((p - low14) / (high14 - low14)) * 100 : null;
    ...
    const mfr = negMF === 0 ? (posMF > 0 ? 100 : 1) : posMF / negMF;
    const mfi14 = 100 - (100 / (1 + mfr));
    let mfv = 0, vol = 0;
    for (const b of last20) {
        const range = Number(b.high) - Number(b.low);
        const mult = range ? ((Number(b.close) - Number(b.low)) - (Number(b.high) - Number(b.close))) / range : 0;
        mfv += mult * Number(b.volume || 0);
    ...
    const obvN = Math.min(10, obvs.length - 1);
    const obvSlope = obvN > 0 ? (obvs.at(-1) - obvs.at(-(obvN + 1))) / Math.max(1, avg(v.slice(-obvN))) : null;
    const vwapDen = v20.reduce((a, b) => a + b, 0), vwap20 = vwapDen ? tp.slice(-20).reduce((sum, x, i) => sum + x * v20[i], 0) / vwapDen : null;
    const vwap20Distance = vwap20 ? ((p / vwap20) - 1) * 100 : null;
    ...
    for (let i = 1; i < last60.length; i++)
        if (Number(last60[i].low) < Number(last60[anchorIdx].low))
            anchorIdx = i;
    const anchored = last60.slice(anchorIdx);
    const avDen = anchored.reduce((sum, b) => sum + Number(b.volume || 0), 0);
    const avwap = avDen ? anchored.reduce((sum, b) => sum + ((Number(b.high) + Number(b.low) + Number(b.close)) / 3) * Number(b.volume || 0), 0) / avDen : null;
    const anchoredVwapDistance = avwap ? ((p / avwap) - 1) * 100 : null;
    const profile = bars.slice(-60);
    const pLo = Math.min(...profile.map(b => Number(b.low))), pHi = Math.max(...profile.map(b => Number(b.high)));
    let profilePoc = null;
    if (pHi > pLo) {
        const bins = 20, vols = Array(bins).fill(0);
        for (const b of profile) {
            const typical = (Number(b.high) + Number(b.low) + Number(b.close)) / 3;
            const idx = Math.min(bins - 1, Math.max(0, Math.floor((typical - pLo) / (pHi - pLo) * bins)));
            vols[idx] += Number(b.volume || 0);
        }
        const idx = vols.indexOf(Math.max(...vols));
        profilePoc = pLo + (idx + .5) * (pHi - pLo) / bins;
    }
    const volumeProfilePocDistance = profilePoc ? ((p / profilePoc) - 1) * 100 : null;
    const bbUpper = mean20 + 2 * sd20, bbLower = mean20 - 2 * sd20;
    const squeezeState = kUpper != null && kLower != null ? (bbUpper < kUpper && bbLower > kLower ? 1 : 0) : null;
...
function buildProfessionalMetrics({ technical, v4, bars = [] }) {
    ...
    for (const [key, meta] of Object.entries(factorMap)) {
