...
exports.analyzeTechnicalPatterns = analyzeTechnicalPatterns;
const clamp = (n, min = 0, max = 100) => Math.max(min, Math.min(max, n));
const conf = (s) => s >= 75 ? 'HIGH' : s >= 55 ? 'MEDIUM' : 'LOW';
function pivots(bars) {
    const lows = [];
    const highs = [];
    for (let i = 2; i < bars.length - 2; i++) {
        const w = bars.slice(i - 2, i + 3);
        if (bars[i].low === Math.min(...w.map(x => x.low)))
            lows.push({ i, p: bars[i].low });
        if (bars[i].high === Math.max(...w.map(x => x.high)))
            highs.push({ i, p: bars[i].high });
    }
    return { lows, highs };
}
function analyzeTechnicalPatterns(bars, technical) {
    if (!technical || bars.length < 20)
        return [];
    const { lows, highs } = pivots(bars);
    const out = [];
    const px = technical.price;
    const last2 = lows.slice(-2);
    const lastHighs = highs.slice(-2);
    if (last2.length === 2) {
        const [a, b] = last2;
        const sep = b.i - a.i;
        const diff = Math.abs(b.p - a.p) / Math.max(a.p, b.p);
    ...
    if (lastHighs.length >= 2 && last2.length >= 2) {
        const hh = lastHighs.at(-1).p >= lastHighs.at(-2).p * .98;
        const hl = last2.at(-1).p >= last2.at(-2).p * .98;
