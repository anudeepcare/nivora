...
exports.assessV93Promotion = assessV93Promotion;
exports.dispositionFor = dispositionFor;
function assessV93Promotion(m, qValue, policy) {
    const checks = {
        evidence: m.totalN > 0,
    ...
    const blockers = Object.keys(checks).filter(k => !checks[k]).map(k => messages[k]);
    return { eligible: blockers.length === 0, checks, blockers };
}
function dispositionFor(m, p) {
    const c = p.checks;
    if (!c.evidence)
        return 'REJECTED_NO_EVIDENCE';
    if (!c.sample || !c.oosSample)
        return 'REJECTED_SAMPLE';
    if (!c.walkForward)
        return 'REJECTED_WALK_FORWARD';
    if (!c.oosAlpha || !c.bootstrap || !c.informationCoefficient || !c.hitRate)
        return 'REJECTED_OOS';
    if (!c.costStress)
        return 'REJECTED_COST';
    if (!c.foldStability || !c.regimeBreadth || !c.regimeStability || !c.archetypeBreadth || !c.archetypeStability || !c.sectorBreadth || !c.sectorStability || !c.drawdown)
        return 'REJECTED_ROBUSTNESS';
    if (!c.falseDiscovery)
        return 'REJECTED_FDR';
