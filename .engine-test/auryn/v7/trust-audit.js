...
exports.auditCanonicalTrust = auditCanonicalTrust;
const finite = (x) => typeof x === 'number' && Number.isFinite(x);
const near = (a, b) => finite(a) && finite(b) && Math.abs(a - b) <= Math.max(.01, Math.abs(a) * .0001);
const buyLike = (a) => a === 'BUY' || a === 'STRONG_BUY';
function auditCanonicalTrust(input) {
    const checks = [];
    ...
    if (plan.state === 'READY' && plan.initialEntry && plan.invalidation != null) {
        ...
        for (const z of plan.dcaZones) {
            orderingOk = orderingOk && z.low <= z.high && z.low > plan.invalidation && z.high < ceiling;
            ceiling = z.low;
        }
        if (plan.confirmation != null)
            orderingOk = orderingOk && plan.confirmation > plan.initialEntry.high;
        let prev = plan.confirmation ?? plan.initialEntry.high;
        for (const t of plan.targets) {
            orderingOk = orderingOk && finite(t.price) && t.price > prev;
            prev = t.price;
        }
    }
    ...
    const state = blockers.length ? 'BLOCK' : warnings.length ? 'WARN' : 'PASS';
