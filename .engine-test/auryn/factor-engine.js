...
exports.buildCanonicalFactors = buildCanonicalFactors;
const score_label_1 = require("./score-label");
...
const renormal = (xs) => { const a = xs.filter(x => x.available !== false && finite(x.value)); const w = a.reduce((s, x) => s + x.weight, 0); return w ? clamp(a.reduce((s, x) => s + x.value * x.weight, 0) / w) : null; };
const factor = (score, coverage, reason) => score == null ? { score: null, label: "N/A", coverage, reason } : { score: Math.round(score), label: (0, score_label_1.scoreLabel)(score), coverage, reason };
function buildCanonicalFactors({ market, company, context, institutional }) {
    const raw = company?.rawMetrics || {}, cls = (0, classification_1.classifySecurity)({ assetType: market?.assetType || company?.assetType, name: company?.name, description: context?.profile?.description, industry: context?.profile?.finnhubIndustry, profile: context?.profile, profitable: finite(raw?.netIncome) ? Number(raw.netIncome) > 0 : undefined });
    ...
    const surpriseVals = surprises.slice(0, 3).map((x) => finite(x?.surprisePercent) ? clamp(50 + Number(x.surprisePercent) * .8) : null).filter((x) => x != null);
