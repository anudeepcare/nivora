...
exports.summarizeCalibration = summarizeCalibration;
...
function wilson(w, n) { if (!n)
    return null; const z = 1.96, p = w / n, d = 1 + z * z / n, c = (p + z * z / (2 * n)) / d, m = z * Math.sqrt((p * (1 - p) + z * z / (4 * n)) / n) / d; return { lowPct: round((c - m) * 100, 1), highPct: round((c + m) * 100, 1) }; }
function summarizeCalibration(rows, minimum = 100) {
    ...
    for (const r of clean) {
        const b = Math.min(9, Math.floor(clamp(r.score / 100) * 10)), x = buckets.get(b) || { n: 0, p: 0, y: 0 };
        x.n++;
        x.p += clamp(r.score / 100);
        x.y += r.alphaPct > 0 ? 1 : 0;
        buckets.set(b, x);
    }
