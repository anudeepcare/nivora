...
exports.buildActionTriggers = buildActionTriggers;
function buildActionTriggers(d) {
    const audit = d.buyAudit;
    const blockers = [];
    if (audit?.blockers?.length)
        blockers.push(...audit.blockers.slice(0, 3));
    else {
        if (d.timingLabel === "WEAK" || d.timingLabel === "OVEREXTENDED" || d.timingScore < 45)
            blockers.push(`Timing is ${d.timingScore}/100 (${d.timingLabel}); price/technical evidence must stabilize.`);
        if (d.thesisScore < 68)
            blockers.push(`Thesis ${d.thesisScore}/100 is below the constructive capital threshold.`);
        if (d.opportunityScore < 62)
            blockers.push(`Opportunity ${d.opportunityScore}/100 is below the staged-capital threshold.`);
        if ((d.vetoes?.length || 0) > 0)
            blockers.push(`Hard veto active: ${d.vetoes[0]}.`);
    }
    ...
    if (d.owns) {
        ...
        if (d.action === "ADD" && audit?.eligible)
            return {
    ...
    if (bearish || (d.vetoes?.length || 0) > 0 || d.action === "AVOID")
        return {
    ...
    if (d.action === "BUY" && audit?.eligible)
        return {
