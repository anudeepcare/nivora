...
exports.roundPriceZone = roundPriceZone;
exports.buildDecisionReality = buildDecisionReality;
exports.technicalRealityFromCandles = technicalRealityFromCandles;
exports.applyRealityGuardToToday = applyRealityGuardToToday;
const clamp = (x, a = 0, b = 100) => Math.max(a, Math.min(b, x));
const finite = (x) => x !== null && x !== undefined && x !== "" && typeof x !== "boolean" && Number.isFinite(Number(x));
const n = (x, f = 0) => finite(x) ? Number(x) : f;
const round = (x, d = 1) => +x.toFixed(d);
function roundPriceZone(zone, confidence, referencePrice) {
    ...
    const r = (v) => v == null ? null : +(Math.round(v / step) * step).toFixed(step < .1 ? 2 : step < 1 ? 2 : 0);
    return { ...zone, low: r(zone.low), high: r(zone.high) };
}
function valuationRobustness(price, range, archetype) {
    if (!range || !finite(price) || price <= 0)
        return { score: 0, label: "UNAVAILABLE", reason: "Decision-grade valuation is unavailable.", stressBear: null };
    ...
    if (bearUpside > 15)
        score -= Math.min(28, (bearUpside - 15) * .55);
    if (baseUpside > 65)
        score -= Math.min(22, (baseUpside - 65) * .30);
    if (dispersion < .12)
        score -= 10;
    if (dispersion > .65)
        score -= 14;
    if (archetype === "hypergrowth" || archetype === "ai_infrastructure" || archetype === "pre_scale")
        score -= 8;
    score = Math.round(clamp(score));
    const stressFactor = archetype === "pre_scale" ? .60 : archetype === "hypergrowth" || archetype === "ai_infrastructure" ? .72 : .80;
    return {
        score,
        label: score >= 75 ? "ROBUST" : score >= 55 ? "SENSITIVE" : "FRAGILE",
...
function marketModelDisagreement(price, range) {
    if (!range || !finite(price) || price <= 0)
        return { level: "LOW", reason: "No decision-grade absolute valuation is available for market/model comparison.", bearUpsidePct: null, baseUpsidePct: null };
    const bear = (n(range.bear) / price - 1) * 100, base = (n(range.base) / price - 1) * 100;
    const level = bear > 20 || base > 80 ? "HIGH" : bear > 10 || base > 50 ? "MEDIUM" : "LOW";
...
function stabilization(technical, timingScore, timingLabel) {
    const reasons = [];
    ...
    let pressure = 0;
    if (timingLabel === "WEAK" || timingScore < 40) {
        pressure += 35;
        reasons.push(`Timing is ${timingScore}/100 (${timingLabel}).`);
    }
    if (d20 < -7) {
        pressure += 18;
        reasons.push(`Price is ${Math.abs(d20).toFixed(1)}% below the 20D trend reference.`);
    }
    if (d50 < -15) {
        pressure += 18;
        reasons.push(`Price is ${Math.abs(d50).toFixed(1)}% below the 50D trend reference.`);
    }
    if (d200 < -25) {
        pressure += 12;
        reasons.push(`Price is ${Math.abs(d200).toFixed(1)}% below the 200D trend reference.`);
    }
    if (drawdown < -35) {
        pressure += 12;
        reasons.push(`Drawdown is ${Math.abs(drawdown).toFixed(1)}% from the recent high.`);
    }
    if (vol >= 1.5 && pressure >= 35) {
        pressure += 8;
        reasons.push("Heavy participation is occurring during a weak price regime.");
    }
    const state = pressure >= 55 ? "REQUIRED" : pressure >= 28 ? "WATCH" : "CONFIRMED";
...
function earlyWarning(input, disagreement, stabilizationState) {
    const f = input.factors || {}, reasons = [];
    let score = 0;
    if (n(f.forward, 50) < 50) {
        score += 24;
        reasons.push("Forward evidence is below neutral.");
    }
    if (n(f.financial, 50) < 45) {
        score += 22;
        reasons.push("Financial quality is below the durable-thesis threshold.");
    }
    if (n(f.risk, 50) >= 70) {
        score += 18;
        reasons.push("Risk pressure is elevated.");
    }
    if (String(input.newsTone || "").toLowerCase() === "negative") {
        score += 18;
        reasons.push("Recent material news is negative.");
    }
    if (disagreement === "HIGH") {
        score += 16;
        reasons.push("Market/model disagreement is high.");
    }
    if (stabilizationState === "REQUIRED") {
        score += 16;
        reasons.push("Price stabilization is required.");
    }
    score = Math.round(clamp(score));
...
function attribution(f, filingRisk = false, vetoCount = 0) {
    const parts = [
    ...
    const rows = parts.map(([label, w, score, evidence]) => {
        const s = finite(score) ? Number(score) : null, impact = s == null ? null : round((s - 50) * w, 1);
        const direction = impact == null || Math.abs(impact) < .5 ? "NEUTRAL" : impact > 0 ? "POSITIVE" : "NEGATIVE";
    ...
    for (const [label, impact, active, evidence] of penalties)
        if (active)
            rows.push({ label, weightPct: 0, score: null, impactPoints: impact, direction: "NEGATIVE", evidence });
    return rows.sort((a, b) => Math.abs(b.impactPoints ?? 0) - Math.abs(a.impactPoints ?? 0));
}
function buildDecisionReality(input) {
...
function technicalRealityFromCandles(market) {
    const candles = Array.isArray(market?.candles) ? market.candles : [], closes = candles.map((c) => Number(c?.close)).filter((x) => Number.isFinite(x) && x > 0);
    const vols = candles.map((c) => Number(c?.volume)).filter((x) => Number.isFinite(x) && x >= 0);
    const last = closes.at(-1) ?? (Number(market?.price) || 0);
    const avg = (xs) => xs.length ? xs.reduce((a, b) => a + b, 0) / xs.length : null;
    const sma = (p) => closes.length >= p ? avg(closes.slice(-p)) : null;
    const d = (v) => v && last ? ((last / v) - 1) * 100 : null;
...
function applyRealityGuardToToday(today, owns, reality) {
    if (today.action !== "BUY" && today.action !== "ADD")
        return today;
    const blockers = [];
    if (reality.stabilization.state === "REQUIRED")
        blockers.push("price stabilization is required");
    if (reality.earlyWarning.level === "HIGH")
        blockers.push("fast-moving early-warning risk is high");
    if (reality.marketModelDisagreement.level === "HIGH" && reality.valuationRobustness.label === "FRAGILE")
        blockers.push("market/model disagreement is high while valuation robustness is fragile");
    if (!blockers.length)
        return today;
    return { ...today, action: (owns ? "HOLD" : "WAIT"), blocked: false, reason: `Reality guard withheld additional risk because ${blockers.join("; ")}.` };
}
