...
export default function PortfolioPerformanceChart({points}:{points:Point[]}){
 ...
 return <div className="aurynPortfolioChart"><div className="aurynChartToggles">{keys.map(k=><button key={k} className={shown[k]?"on":""} onClick={()=>setShown(v=>({...v,[k]:!v[k]}))}>{k==="PORTFOLIO"?"You":k}</button>)}</div><svg viewBox="0 0 100 100" preserveAspectRatio="none" role="img" aria-label="Portfolio performance chart"><line x1="4" y1={90-((0-min)/span)*80} x2="96" y2={90-((0-min)/span)*80}/>{keys.map((k,i)=>shown[k]?<path key={k} className={`series s${i}`} d={path(k)} fill="none" vectorEffect="non-scaling-stroke"/>:null)}</svg><div className="aurynPortfolioChartAxis"><span>{points[0].asOf.slice(0,10)}</span><span>{points[points.length-1].asOf.slice(0,10)}</span></div></div>
}