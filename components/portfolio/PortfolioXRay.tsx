...
export default function PortfolioXRay({pulse,risk}:{pulse:any;risk?:any}){
 ...
 return <section className="aurynPortfolioXRay"><div className="aurynPortfolioSectionHead"><div><small>PORTFOLIO X-RAY</small><h2>Exposure from multiple angles</h2></div></div><nav className="aurynXRayModes">{MODES.map(x=><button key={x} className={mode===x?"on":""} onClick={()=>setMode(x)}>{x}</button>)}</nav>{metrics.length?<div className="aurynXRayRail">{metrics.map(([a,b,c]:any)=><article key={a}><small>{a}</small><b>{b}</b><span>{c}</span></article>)}</div>:<p className="aurynXRayQuiet">This view appears when reliable classification or market evidence exists. AURYN does not invent exposure data.</p>}</section>
}