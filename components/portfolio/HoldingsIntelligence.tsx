...
const usd=(v:any,d=2)=>{const x=Number(v);return Number.isFinite(x)?`$${x.toLocaleString(undefined,{minimumFractionDigits:d,maximumFractionDigits:d})}`:"—"};
...
export default function HoldingsIntelligence({assets,onEdit,onRemove,editingId,editDraft,onEditDraft,onSaveEdit,onCancelEdit,pendingDelete,onConfirmRemove,onCancelRemove}:{assets:any[];onEdit?:(x:any)=>void;onRemove?:(x:any)=>void;editingId?:string|number|null;editDraft?:any;onEditDraft?:(x:any)=>void;onSaveEdit?:()=>void;onCancelEdit?:()=>void;pendingDelete?:string|number|null;onConfirmRemove?:()=>void;onCancelRemove?:()=>void}){
 ...
 return <section id="portfolio-holdings" className="aurynHoldingsSection"><div className="aurynHoldingsHead"><div><div className="aurynEyebrow">Positions</div><h2>Your holdings</h2></div><span>{rows.length} tracked assets</span></div>
  <div className="aurynPositionList">{rows.map((x:any)=>{const cash=x.assetType==="CASH",crypto=x.assetType==="CRYPTO",name=cash?(x.currency||x.symbol||"USD"):x.symbol,weight=total?x.value/total*100:0,current=Number(x.price),avg=Number(x.avgCost),qty=Number(x.quantity||0),pnl=!cash&&avg>0?(current-avg)*qty:null,pnlPct=!cash&&avg>0&&Number.isFinite(current)?(current/avg-1)*100:null,action=cash?"LIQUIDITY":String(x.action||"REVIEW").replaceAll("_"," "),source=x.source||x,isEditing=editingId!=null&&String(source.id)===String(editingId),isDeleting=pendingDelete!=null&&String(source.id)===String(pendingDelete),confirm=levelNum(x.marketIntelligence,"confirm"),invalidate=levelNum(x.marketIntelligence,"invalidation"),nextTrigger=String(x.nextTrigger||"").trim()||(confirm?`Confirm above ${usd(confirm)}`:invalidate?`Reassess below ${usd(invalidate)}`:"Await new evidence");return <div className={`aurynPositionGroup ${isEditing||isDeleting?"active":""}`} key={`${x.assetType}-${name}`}>
