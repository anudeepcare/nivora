...
export default function HoldingsIntelligence({assets,onEdit,onRemove,editingId,editDraft,onEditDraft,onSaveEdit,onCancelEdit,pendingDelete,onConfirmRemove,onCancelRemove}:{assets:any[];onEdit?:(x:any)=>void;onRemove?:(x:any)=>void;editingId?:string|number|null;editDraft?:any;onEditDraft?:(x:any)=>void;onSaveEdit?:()=>void;onCancelEdit?:()=>void;pendingDelete?:string|number|null;onConfirmRemove?:()=>void;onCancelRemove?:()=>void}){
 const router=useRouter(); const[sortKey,setSortKey]=useState<"NAME"|"VALUE"|"PNL"|"PNL_PCT"|"WEIGHT"|"AURYN">("VALUE"),[sortDesc,setSortDesc]=useState(true);
 const baseRows=(assets||[]).map(x=>{const cash=x.assetType==="CASH",amount=finiteNum(x.amount),qty=finiteNum(x.quantity),price=finiteNum(x.price),value=cash?(amount??0):qty!=null&&price!=null?qty*price:finiteNum(x.value)??0,avg=finiteNum(x.avgCost),pnl=!cash&&avg!=null&&price!=null?(price-avg)*(qty??0):0,pnlPct=!cash&&avg!=null&&avg>0&&price!=null?(price/avg-1)*100:0;return{...x,value,_pnl:pnl,_pnlPct:pnlPct}});
 const total=baseRows.reduce((n,x)=>n+x.value,0);
 const rows=[...baseRows].sort((a:any,b:any)=>{const val=(x:any)=>sortKey==="NAME"?String(x.symbol||x.currency||""):sortKey==="VALUE"?x.value:sortKey==="PNL"?x._pnl:sortKey==="PNL_PCT"?x._pnlPct:sortKey==="WEIGHT"?(total?x.value/total:0):String(x.action||"REVIEW");const av=val(a),bv=val(b),c=typeof av==="string"?av.localeCompare(bv):Number(av)-Number(bv);return sortDesc?-c:c});
 const open=(cash:boolean,name:string)=>{if(!cash)router.push(`/stock/${encodeURIComponent(name)}`)};
 return <section id="portfolio-holdings" className="aurynHoldingsSection"><div className="aurynHoldingsHead"><div><div className="aurynEyebrow">Positions</div><h2>Your holdings</h2></div><span>{rows.length} tracked assets</span></div><div className="aurynHoldingSort"><small>SORT</small>{(["NAME","VALUE","PNL","PNL_PCT","WEIGHT","AURYN"] as const).map(k=><button key={k} className={sortKey===k?"on":""} onClick={()=>{if(sortKey===k)setSortDesc(v=>!v);else{setSortKey(k);setSortDesc(k!=="NAME")}}}>{k==="PNL"?"P/L":k==="PNL_PCT"?"P/L %":k}{sortKey===k?(sortDesc?" ↓":" ↑"):""}</button>)}</div>
