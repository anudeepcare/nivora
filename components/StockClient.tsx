...
export default function StockClient({symbol}:{symbol:string}){
  ...
  return <div className="aurynStockPage">
    ...
    {depth==="pro"&&institutionalDecision&&decisionSnapshot?<AstraAnalystPanel symbol={symbol} snapshotId={decisionSnapshot.snapshotId} canonicalAction={String(v5Analysis?.decision.primaryAction||institutionalDecision.newMoneyAction)} evidence={institutionalEvidence}/>:null}
    {v5Analysis?<>{canonicalTrustBlocked?<div className="aurynIntegrityAlert aurynTrustBlock" role="alert"><b>CANONICAL TRUST BLOCK</b><span>{v7Analysis?.trust.blockers[0]||"AURYN detected an internal snapshot/plan inconsistency."} Price-sensitive execution levels are suppressed until the canonical chain is aligned.</span></div>:<ExecutionPlanPanel plan={v5Analysis.executionPlan}/>}
    {depth==="pro"&&v5Analysis?<details className="v931LegacyDiagnostics"><summary>Extreme Pro · model diagnostics & scenario map</summary><div className="v931LegacyDiagnosticsBody"><StockV5Decision snapshot={v5Analysis} v6={v6Analysis} v7={v7Analysis} owns={owns} depth={depth} onDepthChange={setDepth}/>{!canonicalTrustBlocked?<ScenarioMapPanel scenario={v5Analysis.scenario} mode="compact"/>:null}</div></details>:null}</>:<section className="aurynV5Unavailable"><small>AURYN V8 CANONICAL ANALYSIS</small><b>COLLECTING VERIFIED EVIDENCE</b><span>AURYN will not fall back to a second decision engine while the canonical snapshot is unavailable.</span></section>}
