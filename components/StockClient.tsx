      ...
      {tab==="thesis"&&presentedDecision&&<div className="v65Thesis">
        <div className="v658VerdictSurface">
          ...
          <div className="v658VerdictMetrics">
            <article><div><small>THESIS <MetricInfo title="Thesis score" score={presentedDecision.thesisScore}>Overall alignment of the business, forward evidence and durability behind the investment case.</MetricInfo></small><b>{presentedDecision.thesisScore}/100</b></div><i><em style={{width:`${presentedDecision.thesisScore}%`}}/></i><span>{presentedDecision.thesisLabel} overall evidence</span></article>
            {presentedDecision.longTermThesis&&<article><div><small>LONG-TERM <MetricInfo title="Long-term thesis" score={presentedDecision.longTermThesis.score}>Separates 1–3 year business evidence from near-term price action.</MetricInfo></small><b>{presentedDecision.longTermThesis.score}/100</b></div><i><em style={{width:`${presentedDecision.longTermThesis.score}%`}}/></i><span>{presentedDecision.longTermThesis.label} · {presentedDecision.longTermThesis.longTerm}</span></article>}
            {presentedDecision.expectationGap&&<article><div><small>EXPECTATION GAP <MetricInfo title="Expectation gap" score={presentedDecision.expectationGap.score??undefined}>Compares forward growth, earnings/revision direction and catalysts with a neutral baseline. It is not a price target.</MetricInfo></small><b>{presentedDecision.expectationGap.score!=null?`${presentedDecision.expectationGap.score}/100`:"—"}</b></div>{presentedDecision.expectationGap.score!=null&&<i><em style={{width:`${presentedDecision.expectationGap.score}%`}}/></i>}<span>{presentedDecision.expectationGap.label} · {presentedDecision.expectationGap.reason}</span></article>}
          </div>
        </div>
