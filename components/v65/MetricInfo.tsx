...
export default function MetricInfo({metric,title,description,children}:{metric?:keyof typeof metricDefinitions;title?:string;description?:string;proof?:MetricProof;children?:React.ReactNode}){
 ...
 return <span className="v65MetricInfo" ref={ref}>
  ...
  {open?<span id={id} className="v65MetricSheet v655MetricSheet" role="tooltip"><b>{label}</b><p>{body}</p>{def?.uses?<span className="v653Why"><strong>Why it matters</strong>{def.uses}</span>:null}{children?<div className="v65MetricCustom">{children}</div>:null}</span>:null}
 </span>;
}
