...
export default function MetricInfo({metric,title,description,children}:{metric?:keyof typeof metricDefinitions;title?:string;description?:string;proof?:MetricProof;children?:React.ReactNode}){
 ...
 return <span className="v65MetricInfo" ref={ref}>
  <button type="button" aria-label={`About ${label}`} aria-expanded={open} aria-controls={id} onClick={e=>{e.stopPropagation();setOpen(v=>!v)}}>i</button>
  {open?<span id={id} className="v65MetricSheet v656MetricSheet" role="tooltip"><b>{label}</b><p>{body}</p>{def?.uses?<span className="v653Why"><strong>Why it matters</strong>{def.uses}</span>:null}{children?<div className="v65MetricCustom">{children}</div>:null}</span>:null}
 </span>;
}
