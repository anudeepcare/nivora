...
export default function InvestorDecisionHero({
 ...
 return <section className="v65Cockpit v65CockpitConsistency v65DecisionCockpit v653DecisionFirst">
  ...
  {buyAudit?<div className={`v653BuyPath ${buyAudit.eligible?"eligible":"waiting"}`}><div><small>{buyAudit.eligible?"WHY IT QUALIFIES":"WHAT IS MISSING"}</small><b>{buyAudit.eligible?"Ready for staged new capital":"Not ready for new capital yet"}</b><span>{buyAudit.eligible?"The business case, company quality and current setup all cleared the minimum required for this buy path.":buyAudit.primaryBlocker||confirmation}</span></div><div className="v655Qualification"><strong>To qualify for new capital</strong><span><b>Business case</b><em>{formatScore(decision.thesisScore)}/100</em><small>72+ strong long-term case</small></span><span><b>Company quality</b><em>{formatScore(decision.companyScore)}/100</em><small>65+ strong enough business quality</small></span><span className={decision.opportunityScore>=64?"pass":"miss"}><b>Current setup</b><em>{formatScore(decision.opportunityScore)}/100</em><small>64+ attractive enough setup</small>{decision.opportunityScore<64?<i>{formatScore(decision.opportunityScore)}/100 = below the 64 needed today</i>:<i>Threshold cleared</i>}</span></div></div>:null}
