...
export default function InvestorDecisionHero({
 ...
 return <section className="v65Cockpit v65CockpitConsistency v65DecisionCockpit v653DecisionFirst aurynZeroScroll">
  ...
  <div className="v653QuickPlan">
   ...
   <article><small>IF YOU OWN IT · EXISTING HOLDERS</small><b className={tone(guidance.owner)}>{guidance.owner}</b><span>{ownerSummary}</span></article>
  ...
  {buyAudit?(()=>{const gates=[{name:"Business case",score:decision.thesisScore,need:72,metric:"thesis" as const},{name:"Company quality",score:decision.companyScore,need:65,metric:"business" as const},{name:"Current setup",score:decision.opportunityScore,need:64,metric:"opportunity" as const}];const remaining=gates.filter(g=>g.score<g.need).length;return <div className={`v658DecisionGate ${buyAudit.eligible?"eligible":"waiting"}`}><div className="v658GateLead"><small>{buyAudit.eligible?"NEW CAPITAL READY":"To qualify for new capital"}</small><b>{buyAudit.eligible?"Ready for staged new capital":`${remaining} condition${remaining===1?"":"s"} remaining`}</b><span>{buyAudit.eligible?"All minimum decision gates cleared.":buyAudit.primaryBlocker||confirmation}</span></div><div className="v658GateChecks">{gates.map(g=>{const pass=g.score>=g.need,gap=Math.max(0,g.need-Math.round(g.score));return <article key={g.name} className={pass?"pass":"miss"}><div><span>{g.name}<MetricInfo metric={g.metric} score={g.score}/></span><b>{formatScore(g.score)}</b></div><small>{pass?`Qualified · minimum ${g.need}`:`${gap} point${gap===1?"":"s"} below qualification · minimum ${g.need}`}</small></article>})}</div></div>})():null}

  <details className="v653DeepResearch v654Research"><summary><div><span>Explore full analysis</span><small>See the evidence behind this decision</small></div><strong>Open ↓</strong></summary><div className="v653DeepBody"><nav className="v654ResearchNav v655ResearchChoice" aria-label="Research sections"><a href="#v654-fundamentals">Fundamentals</a><a href="#v654-valuation">Valuation</a><a href="#v654-technicals">Technicals</a><a href="#v654-risks">Risks</a><a href="#v654-outlook">Outlook</a></nav>
   <div className="v65ScoreStrip" id="v654-fundamentals">
    <article><small>COMPANY QUALITY <MetricInfo metric="business" score={decision.companyScore} proof={proof.business}/></small><b>{formatScore(decision.companyScore)}/100</b><span>{decision.companyLabel}</span></article>
    <article><small>CURRENT OPPORTUNITY <MetricInfo metric="opportunity" score={decision.opportunityScore} proof={proof.opportunity}/></small><b>{formatScore(decision.opportunityScore)}/100</b><span>{decision.valuationValidity?.fairValueAllowed?decision.valuationLabel:"Valuation not established"}</span></article>
    <article><small>TIMING <MetricInfo metric="timing" score={decision.timing.score} proof={proof.timing}/></small><b>{formatScore(decision.timing.score)}/100</b><span>{decision.timing.label}</span></article>
   </div>
