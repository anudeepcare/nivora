...
export default function InvestorDecisionHero({
 ...
 return <section className="v65Cockpit v65CockpitConsistency v65DecisionCockpit v653DecisionFirst">
  ...
  {buyAudit?<div className={`v653BuyPath ${buyAudit.eligible?"eligible":"waiting"}`}><div><small>{buyAudit.eligible?"WHY IT QUALIFIES":"WHAT IS MISSING"}</small><b>{buyAudit.eligible?"Ready for staged new capital":"Not ready for new capital yet"}</b><span>{buyAudit.eligible?"The business case, company quality and current setup all cleared the minimum required for this buy path.":buyAudit.primaryBlocker||confirmation}</span></div><div className="v655Qualification"><strong>To qualify for new capital</strong><span><b>Business case</b><em>{formatScore(decision.thesisScore)}/100</em><small>Long-term business + durability + forward evidence. 72+ is the standard strong-case threshold.</small></span><span><b>Company quality</b><em>{formatScore(decision.companyScore)}/100</em><small>Business strength independent of today's price. 65+ is the minimum quality floor.</small></span><span className={decision.opportunityScore>=64?"pass":"miss"}><b>Current setup</b><em>{formatScore(decision.opportunityScore)}/100</em><small>Price + timing + valuation + risk for new money. 64+ means attractive enough to consider.</small>{decision.opportunityScore<64?<i>{formatScore(decision.opportunityScore)}/100 is {Math.max(0,64-Math.round(decision.opportunityScore))} point{Math.max(0,64-Math.round(decision.opportunityScore))===1?"":"s"} short.</i>:<i>Setup threshold cleared.</i>}</span></div></div>:null}
