...
export default function InvestorDecisionHero({
 ...
 return <section className="v65Cockpit v65CockpitConsistency v65DecisionCockpit v653DecisionFirst">
  ...
  {buyAudit?(()=>{const gates=[{name:"Business case",score:decision.thesisScore,need:72,metric:"thesis" as const},{name:"Company quality",score:decision.companyScore,need:65,metric:"business" as const},{name:"Current setup",score:decision.opportunityScore,need:64,metric:"opportunity" as const}];const remaining=gates.filter(g=>g.score<g.need).length;return <div className={`v658DecisionGate ${buyAudit.eligible?"eligible":"waiting"}`}><div className="v658GateLead"><small>DECISION GATE</small><b>{buyAudit.eligible?"Ready for staged new capital":`${remaining} condition${remaining===1?"":"s"} remaining`}</b><span>{buyAudit.eligible?"All minimum decision gates cleared.":buyAudit.primaryBlocker||confirmation}</span></div><div className="v658GateChecks">{gates.map(g=>{const pass=g.score>=g.need,gap=Math.max(0,g.need-Math.round(g.score));return <article key={g.name} className={pass?"pass":"miss"}><div><span>{g.name}<MetricInfo metric={g.metric} score={g.score}/></span><b>{formatScore(g.score)}<em> / {g.need}</em></b></div><small>{pass?"Passed":`${gap} point${gap===1?"":"s"} to clear`}</small></article>})}</div></div>})():null}
