...
export default function SearchBox({large=false,compact=false}:{large?:boolean;compact?:boolean}){const[q,setQ]=useState(""),[remote,setRemote]=useState<R[]>([]),[open,setOpen]=useState(false),[busy,setBusy]=useState(false),r=useRouter();
 ...
 return <div className={`osSearch ${large?"large":""} ${compact?"compact":""}`}><form onSubmit={submit}><Search size={20}/><input aria-label="Search investments" value={q} onFocus={()=>setOpen(true)} onChange={e=>{setQ(e.target.value);setOpen(true)}} placeholder="Company, ticker or crypto…"/><button disabled={busy} aria-label="Analyze">{busy?"Opening…":<><span>{compact?"Go":"Analyze"}</span><ArrowRight size={17}/></>}</button></form>{open&&items.length>0&&<div className="osSearchResults">{items.map(x=><button type="button" key={`${x.symbol}-${x.exchange}`} onClick={()=>choose(x)}><span><b>{x.name}</b><small>{x.exchange||x.type||""}</small></span><strong>{x.symbol}</strong></button>)}</div>}</div>}
