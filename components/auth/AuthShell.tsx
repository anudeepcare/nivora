...
export default function AuthShell({eyebrow,title,subtitle,storyTitle,storyBody,points,children}:{eyebrow:string;title:string;subtitle:string;storyTitle:string;storyBody:string;points:string[];children:React.ReactNode}){
 return <main className="aurynAuthPage"><section className="aurynAuthShell">
  ...
  <div className="aurynAuthPanel"><div className="aurynAuthMobileBrand"><AurynLogo href="/"/></div><div className="aurynEyebrow">{eyebrow}</div><h1>{title}</h1><p>{subtitle}</p>{children}<div className="aurynAuthProductLinks"><Link href="/about">About</Link><Link href="/methodology">Methodology</Link><Link href="/terms">Terms</Link><Link href="/privacy">Privacy</Link><Link href="/disclaimer">Risk disclosure</Link></div></div>
 </section></main>
}