 ...
 return <div className="aurynStockTabPage aurynThesis">
  <section className="aurynThesisVerdict">
   ...
   <div className="aurynThesisScoreRail">
    ...
    <article><div><span>Moat strength / durability <MetricInfo title="Moat strength / durability">Competitive durability is shown only when evidence exists. Heuristic evidence remains clearly labeled in Extreme Pro.</MetricInfo></span><b>{moatScore==null?"N/A":`${Math.round(moatScore)}/100 · ${formatScoreBand(moatScore)}`}</b></div>{moatScore!=null&&<i><em style={{width:`${Math.max(3,Math.min(100,moatScore))}%`}}/></i>}<small>Moat trend: {humanize(v4.moat.direction)}. {moatMetric?.interpretation||"Competitive durability evidence unavailable."}</small></article>
