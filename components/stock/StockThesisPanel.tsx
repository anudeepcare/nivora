...
export default function StockThesisPanel({decision,v5,marketTruth}:{decision:any;metricDefinitions?:any;v5:CanonicalAnalysisSnapshot;marketTruth?:any}){
 ...
 return <div className="aurynStockTabPage aurynThesis">
  <section className="aurynThesisVerdict">
   <div className="aurynThesisLead"><div className="aurynEyebrow">Canonical long-term thesis</div><div className="aurynThesisHeadline"><h2>{v4.thesis.direction.replaceAll("_"," ")}</h2><span>{thesisScore==null?"N/A":`${Math.round(thesisScore)}/100 · ${formatScoreBand(thesisScore)}`}</span></div><p>{v4.thesis.companyState}. This tab explains the same V6 decision shown above; it does not calculate a second verdict.</p></div>
