...
import {House,Search,Star,BriefcaseBusiness,UserRound,Bell,Info,ShieldCheck} from "lucide-react";
...
export default function AppShell({children}:{children:React.ReactNode}){
 ...
 return <>
  <header className="osHeader v12Header">
   ...
   <div className="osAccount" ref={ref}><button onClick={()=>setOpen(!open)} aria-label="Account">{(email?.[0]||"N").toUpperCase()}</button>{open&&<div className="osAccountMenu"><small>{email}</small><Link href="/profile"><UserRound size={14}/>Account</Link><Link href="/alerts"><Bell size={14}/>Alerts</Link><Link href="/about"><Info size={14}/>Why NIVORA</Link><Link href="/terms"><ShieldCheck size={14}/>Legal & privacy</Link><button onClick={logout}>Log out</button></div>}</div>
  </header>
