...
export default function AurynResearchOverview({decision,marketTruth,displayPrice,displayPriceLive=false,marketIntelligence,scenario,fundamentalScenario=null,entryQuality,candles,chartLevels}:{decision:InstitutionalDecision;marketTruth:any;displayPrice?:number|null;displayPriceLive?:boolean;marketIntelligence?:any;scenario?:ScenarioMap|null;fundamentalScenario?:any|null;entryQuality?:number|null;candles:any[];chartLevels:any|null}){
 ...
 return <section className="v936Overview" data-snapshot-id={decision.snapshotId}>
  ...
  <section className="v9998AnalystClocks" aria-label="AURYN analyst view">
   <article><header><small>1. BUSINESS QUALITY</small><b>{companyScore==null?"N/A":`${Math.round(companyScore)}/100`}</b></header><h3>{companyBand(companyScore)}</h3><p>{businessTrajectory}</p><span>{decision.pillars.business.why||"Fundamental business evidence is still building."}</span><div className="v9999ClockMetrics"><i><small>Growth / quality</small><b>{companyScore==null?"N/A":Math.round(companyScore)}</b></i><i><small>Forward evidence</small><b>{n(decision.pillars.earningsRevisions.score)==null?"N/A":Math.round(n(decision.pillars.earningsRevisions.score)!)}</b></i><i><small>Evidence</small><b>{decision.evidenceCompleteness}/100</b></i></div></article>
   <article><header><small>2. VALUATION</small><b>{valuationScore==null?"N/A":`${Math.round(valuationScore)}/100`}</b></header><h3>{valueBand(valuationScore)}</h3><p>{marginSafety}</p><span>{decision.pillars.valuation.why||"Independent valuation evidence is still building."}</span><div className="v9999ClockMetrics"><i><small>Valuation</small><b>{valuationScore==null?"N/A":Math.round(valuationScore)}</b></i><i><small>Margin of safety</small><b>{fundamentalScenario?"Measured":"N/A"}</b></i><i><small>Evidence</small><b>{decision.pillars.valuation.state}</b></i></div></article>
