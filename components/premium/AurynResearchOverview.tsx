...
export default function AurynResearchOverview({decision,marketTruth,displayPrice,displayPriceLive=false,marketIntelligence,scenario,fundamentalScenario=null,analystFundamentals=null,entryQuality,candles,chartLevels}:{decision:InstitutionalDecision;marketTruth:any;displayPrice?:number|null;displayPriceLive?:boolean;marketIntelligence?:any;scenario?:ScenarioMap|null;fundamentalScenario?:any|null;analystFundamentals?:any|null;entryQuality?:number|null;candles:any[];chartLevels:any|null}){
 ...
 const valuationScore=n(decision.pillars.valuation.score??analystFundamentals?.valuationScore);
 const timingScore=entryScore!=null?entryScore:n(decision.pillars.marketStructure.score);
 const companyDimensions=analystFundamentals?.companyDimensions??{};
 const fundamentalFairValue=n(analystFundamentals?.fairValue);
 const fundamentalMargin=n(analystFundamentals?.marginOfSafety);
 const businessTrajectory=companyScore==null?"Evidence history pending":`Business trajectory · ${pretty(analystFundamentals?.trajectory||"STABLE")}`;
 const marginSafety=fundamentalMargin==null?(valuationScore==null?"Valuation evidence building":`Valuation context · ${valueBand(valuationScore)}`):`Margin of safety · ${fundamentalMargin>=0?"+":""}${fundamentalMargin.toFixed(1)}%`;
 ...
 return <section className="v936Overview" data-snapshot-id={decision.snapshotId}>
  ...
  <section className="v9998AnalystClocks" aria-label="AURYN analyst view">
   <article><header><small>1. BUSINESS QUALITY</small><b>{companyScore==null?"N/A":`${Math.round(companyScore)}/100`}</b></header><h3>{companyBand(companyScore)}</h3><p>{businessTrajectory}</p><span>{decision.pillars.business.why||"Fundamental business evidence is still building."}</span><div className="v9999ClockMetrics"><i><small>Growth</small><b>{n(companyDimensions.growth)==null?"—":Math.round(n(companyDimensions.growth)!)}</b></i><i><small>Profitability</small><b>{n(companyDimensions.profitability)==null?"—":Math.round(n(companyDimensions.profitability)!)}</b></i><i><small>Cash generation</small><b>{n(companyDimensions.cashGeneration)==null?"—":Math.round(n(companyDimensions.cashGeneration)!)}</b></i><i><small>Balance sheet</small><b>{n(companyDimensions.balanceSheet)==null?"—":Math.round(n(companyDimensions.balanceSheet)!)}</b></i><i><small>Durability</small><b>{n(companyDimensions.durability)==null?"—":Math.round(n(companyDimensions.durability)!)}</b></i><i><small>Forward evidence</small><b>{n(companyDimensions.forwardEvidence)==null?"—":Math.round(n(companyDimensions.forwardEvidence)!)}</b></i></div></article>
   <article><header><small>2. VALUATION</small><b>{valuationScore==null?"N/A":`${Math.round(valuationScore)}/100`}</b></header><h3>{valueBand(valuationScore)}</h3><p>{marginSafety}</p><span>{decision.pillars.valuation.why||"Independent valuation evidence is still building."}</span><div className="v9999ClockMetrics"><i><small>Valuation</small><b>{valuationScore==null?"—":Math.round(valuationScore)}</b></i><i><small>Fair value</small><b>{fundamentalFairValue==null?"Building":money(fundamentalFairValue)}</b></i><i><small>Margin of safety</small><b>{fundamentalMargin==null?"Building":`${fundamentalMargin>=0?"+":""}${fundamentalMargin.toFixed(1)}%`}</b></i></div></article>
   <article><header><small>3. MARKET TIMING</small><b>{timingScore==null?"N/A":`${Math.round(timingScore)}/100`}</b></header><h3>{timingBand(timingScore)}</h3><p>{marketSetup}</p><span>{decision.pillars.marketStructure.why||"Confirmed market structure is still building."}</span><div className="v9999ClockMetrics"><i><small>Market structure</small><b>{n(decision.pillars.marketStructure.score)==null?"N/A":Math.round(n(decision.pillars.marketStructure.score)!)}</b></i><i><small>Entry quality</small><b>{entryScore==null?"N/A":Math.round(entryScore)}</b></i><i><small>Participation</small><b>{participation==null?"N/A":Math.round(participation)}</b></i><i><small>Relative strength</small><b>{relPct==null?"N/A":`${relPct>=0?"+":""}${relPct.toFixed(1)}%`}</b></i><i><small>Volatility</small><b>{volatility||"N/A"}</b></i></div></article>
  </section>

  <section className="v9999DecisionVisuals">
   <div className="v9999ScenarioColumn">
   <article className="v936ScenarioCard v941ScenarioCard v9998FundamentalScenario"><div className="v936CardTitle"><small>VALUATION OUTLOOK</small><b>Scenario spectrum</b><span>Fundamental value scenarios · not probabilities</span></div>
    {fundamentalScenario?<div className="v9998ScenarioReady"><span><small>BEAR</small><b>{money(fundamentalScenario.bear?.value)}</b></span><span><small>BASE</small><b>{money(fundamentalScenario.base?.value)}</b></span><span><small>BULL</small><b>{money(fundamentalScenario.bull?.value)}</b></span><p>{fundamentalScenario.basis||"Independent fundamental valuation"}</p><div className="v99910ScenarioAssumptions"><span>Revenue growth <b>{fundamentalScenario.assumptions?.observedRevenueGrowthPct==null?"—":`${Number(fundamentalScenario.assumptions.observedRevenueGrowthPct).toFixed(1)}%`}</b></span><span>FCF <b>{money(fundamentalScenario.assumptions?.currentFcf)}</b></span><span>Base discount rate <b>{fundamentalScenario.assumptions?.baseDiscountRate}%</b></span><span>Terminal growth <b>{fundamentalScenario.assumptions?.baseTerminalGrowth}%</b></span><span>Confidence <b>{fundamentalScenario.confidence}/100</b></span></div></div>:<div className="v9998ScenarioUnavailable"><b>Fundamental valuation is building</b><span>AURYN needs positive source-backed free cash flow, revenue history, market capitalization and a verified price before publishing Bear / Base / Bull fair values. Technical targets are never substituted.</span></div>}
   </article>
   </div>
   <div className="v9999DecisionColumn">
