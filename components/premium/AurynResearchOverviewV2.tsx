...
export default function AurynResearchOverviewV2({decision,marketTruth,displayPrice,displayPriceLive=false,marketIntelligence=null,fundamentalScenario=null,analystFundamentals=null,entryQuality=null,candles=[],chartLevels=null}:{decision:InstitutionalDecision;marketTruth:any;displayPrice?:number|null;displayPriceLive?:boolean;marketIntelligence?:any;fundamentalScenario?:any;analystFundamentals?:any;entryQuality?:number|null;candles:any[];chartLevels:any}){
 const current=num(displayPrice??marketTruth?.price), business=num(decision.pillars.business.score), valuation=num(analystFundamentals?.valuationScore??decision.pillars.valuation.score), timing=num(entryQuality??decision.pillars.marketStructure.score);
 const dims=analystFundamentals?.companyDimensions||{}, margin=num(analystFundamentals?.marginOfSafety), fair=num(analystFundamentals?.fairValue);
 ...
 const timingMetrics=[
  ["Structure",num(decision.pillars.marketStructure.score)?.toFixed(0)||"—"],
  ["Entry",timing?.toFixed(0)||"—"],
  ["Participation",num(marketIntelligence?.timeframes?.daily?.participationScore??marketIntelligence?.participation)?.toFixed(0)||"—"],
  ["Relative strength",num(marketIntelligence?.relativeStrengthPct)!=null?`${Number(marketIntelligence.relativeStrengthPct)>=0?"+":""}${Number(marketIntelligence.relativeStrengthPct).toFixed(1)}%`:"—"],
  ["Volatility",marketIntelligence?.volatilityLabel||"—"]
 ];
 return <section className="aurynOverviewV2">
  ...
  <div className="v2AnalystClocks">
   ...
   <article><header><small>2. VALUATION</small><b>{valuation==null?"BUILDING":`${valuation.toFixed(0)}/100`}</b></header><h3>{valueBand(valuation)}</h3><p>{margin==null?"Fundamental evidence is still building":`Margin of safety · ${margin>=0?"+":""}${margin.toFixed(1)}%`}</p><div className="v2MetricStrip v2ValueStrip"><span><small>Method</small><b>{fundamentalScenario?.basis?"DCF / FCF":"Building"}</b></span><span><small>Fair value</small><b>{money(fair)}</b></span><span><small>Margin</small><b>{margin==null?"—":`${margin.toFixed(1)}%`}</b></span><span><small>Confidence</small><b>{fundamentalScenario?.confidence!=null?`${fundamentalScenario.confidence}/100`:"—"}</b></span></div></article>
