...
const helpCopy:any={
 ...
 longTerm:"Long-term structure uses independent 5-year history aggregated to weekly bars: WMA/HMA trend, validated swing/Fibonacci context, volume participation, accumulation/distribution, consolidation and structural invalidation.",
 ...
 return <section className="aurynOverviewV2">
  ...
  <section className="v99925Roadmap">
   ...
   {longTerm?.state==="BUILDING"?<div className="v2Building"><b>Long-term structure building</b><span>{longTerm.reason}</span></div>:<>
    <div className="v99925RoadmapRail">{roadmapLevels.map(([label,value,tone]:any,i)=><div className={`v99925RoadmapLevel ${tone}`} key={`${label}-${i}`}><i/><span><small>{label}</small><b>{money(value)}</b></span></div>)}</div>
    <div className="v99925RoadmapEvidence"><span><small>Fib .382</small><b>{money(longTerm.fib382)}</b></span><span><small>Fib .50</small><b>{money(longTerm.fib50)}</b></span><span><small>Fib .618</small><b>{money(longTerm.fib618)}</b></span><span><small>Fib .786</small><b>{money(longTerm.fib786)}</b></span><span><small>50-WMA</small><b>{money(longTerm.wma50)}</b></span><span><small>200-WMA</small><b>{money(longTerm.wma200)}</b></span></div>
    <div className="v99925Wave"><div><small>WAVE / CYCLE CONTEXT</small><b>Wave candidate · {pretty(longTerm.waveCandidate)}</b><span>{longTerm.waveConfidence}% confidence</span></div><p>Wave interpretation is secondary evidence, not a forecast. Fibonacci levels are published as structural zones only when anchored to validated weekly price history.</p></div>
   </>}
  </section>
